...
impl TransitEngine {
    ...
    async fn generate_key(&mut self, name: &str, key_type: KeyType) -> SecretResult<()> {
        ...
        let key_size = match key_type {
            ...
            _ => return Err(SecretError::InvalidConfiguration(format!("Unsupported key type: {:?}", key_type))),
        };
    ...
    async fn encrypt_data(&self, key_name: &str, plaintext: &[u8]) -> SecretResult<String> {
        let key = self.keys.get(key_name)
            .ok_or_else(|| SecretError::InvalidConfiguration(format!("Key '{}' not found", key_name)))?
            .clone();
    ...
    async fn decrypt_data(&self, key_name: &str, ciphertext: &str) -> SecretResult<Vec<u8>> {
        ...
        if combined.len() < 12 {
            return Err(SecretError::InvalidSecretData("Ciphertext too short".to_string()));
        }
...
impl SecretEngine for TransitEngine {
    ...
    async fn read(&self, path: &str) -> SecretResult<Option<Secret>> {
        ...
        if path.starts_with("decrypt/") {
            ...
            return Err(SecretError::InvalidKeyOperation("Decryption requires ciphertext parameter".to_string()));
        }
    ...
    async fn write(&mut self, path: &str, data: HashMap<String, Value>) -> SecretResult<Secret> {
        ...
        // Handle encryption requests
            ...
            let plaintext = data.get("plaintext")
                .and_then(|v| v.as_str())
                .ok_or_else(|| SecretError::InvalidKeyOperation("Missing plaintext parameter".to_string()))?;

            let plaintext_bytes = general_purpose::STANDARD.decode(plaintext)
                .map_err(|e| SecretError::InvalidSecretData(format!("Invalid base64 plaintext: {}", e)))?;
        ...
        // Handle decryption requests
            ...
            let ciphertext = data.get("ciphertext")
                .and_then(|v| v.as_str())
                .ok_or_else(|| SecretError::InvalidKeyOperation("Missing ciphertext parameter".to_string()))?;

            let plaintext_bytes = self.decrypt_data(key_name, ciphertext).await?;

            let mut result_data = HashMap::new();
            result_data.insert("plaintext".to_string(), Value::String(general_purpose::STANDARD.encode(&plaintext_bytes)));
            ...
            Err(SecretError::InvalidKeyOperation("Invalid transit path".to_string()))
        }
    }
