    ...
    pub async fn log_event(&self, event: SecurityEventType) {
        let (core_type, status, user, resource, op, metadata) = match event {
            SecurityEventType::SecretAccess { secret_path, user, action } => 
                (CoreAuditEventType::SecretRead, AuditStatus::Success, user, secret_path, action, None),
            SecurityEventType::SecretCreation { secret_path, user } =>
                (CoreAuditEventType::SecretWrite, AuditStatus::Success, user, secret_path, "create".to_string(), None),
            SecurityEventType::SecretVersionChange { secret_path, old_version, new_version, user } =>
                (CoreAuditEventType::SecretWrite, AuditStatus::Success, user, secret_path, "update_version".to_string(), 
                 Some(vec![("old_version".to_string(), old_version.to_string()), ("new_version".to_string(), new_version.to_string())].into_iter().collect())),
            SecurityEventType::SecretDeletion { secret_path, user } =>
                (CoreAuditEventType::SecretDelete, AuditStatus::Success, user, secret_path, "delete".to_string(), None),
            SecurityEventType::KeyGeneration { key_type, key_id, algorithm, user } =>
                (CoreAuditEventType::Custom("key.generate".to_string()), AuditStatus::Success, user, key_id, "generate".to_string(),
                 Some(vec![("key_type".to_string(), key_type.to_string()), ("algorithm".to_string(), algorithm.to_string())].into_iter().collect())),
            SecurityEventType::KeyRotation { old_key_id, new_key_id, algorithm, user } =>
                (CoreAuditEventType::Custom("key.rotate".to_string()), AuditStatus::Success, user, new_key_id, "rotate".to_string(),
                 Some(vec![("old_key_id".to_string(), old_key_id), ("algorithm".to_string(), algorithm.to_string())].into_iter().collect())),
            SecurityEventType::EncryptionOperation { key_id, user, data_size } =>
                (CoreAuditEventType::Custom("crypto.encrypt".to_string()), AuditStatus::Success, user, key_id, "encrypt".to_string(),
                 Some(vec![("data_size".to_string(), data_size.to_string())].into_iter().collect())),
            SecurityEventType::DecryptionOperation { key_id, user, data_size } =>
                (CoreAuditEventType::Custom("crypto.decrypt".to_string()), AuditStatus::Success, user, key_id, "decrypt".to_string(),
                 Some(vec![("data_size".to_string(), data_size.to_string())].into_iter().collect())),
            SecurityEventType::AuthenticationSuccess { user, method } =>
                (CoreAuditEventType::AuthLogin, AuditStatus::Success, user, "auth".to_string(), "login".to_string(),
                 Some(vec![("method".to_string(), method)].into_iter().collect())),
            SecurityEventType::AuthenticationFailure { user, method, reason } =>
                (CoreAuditEventType::AuthLogin, AuditStatus::Failure, user, "auth".to_string(), "login".to_string(),
                 Some(vec![("method".to_string(), method), ("reason".to_string(), reason)].into_iter().collect())),
            SecurityEventType::SessionTerminated { user, session_id, reason } =>
                (CoreAuditEventType::AuthLogout, AuditStatus::Success, user, "session".to_string(), "terminate".to_string(),
                 Some(vec![("session_id".to_string(), session_id), ("reason".to_string(), reason)].into_iter().collect())),
            SecurityEventType::MfaSetup { user, method } =>
                (CoreAuditEventType::Custom("mfa.setup".to_string()), AuditStatus::Success, user, "mfa".to_string(), "setup".to_string(),
                 Some(vec![("method".to_string(), method)].into_iter().collect())),
            SecurityEventType::MFASuccess { user, method } =>
                (CoreAuditEventType::Custom("mfa.verify".to_string()), AuditStatus::Success, user, "mfa".to_string(), "verify".to_string(),
                 Some(vec![("method".to_string(), method)].into_iter().collect())),
            SecurityEventType::MFARemoval { user, method } =>
                (CoreAuditEventType::Custom("mfa.remove".to_string()), AuditStatus::Success, user, "mfa".to_string(), "remove".to_string(),
                 Some(vec![("method".to_string(), method)].into_iter().collect())),
            SecurityEventType::LoginSuccess { user, method, ip_address } =>
                (CoreAuditEventType::AuthLogin, AuditStatus::Success, user, "auth".to_string(), "login".to_string(),
                 Some(vec![("method".to_string(), method), ("ip_address".to_string(), ip_address.unwrap_or_default())].into_iter().collect())),
...
pub enum SecurityEventType {
    SecretAccess { secret_path: String, user: String, action: String },
    ...
    KeyGeneration { key_type: String, key_id: String, algorithm: String, user: String },
    ...
    PolicyChange { policy_name: String, user: String, action: String },
    ConfigChange { user: String, changed_keys: Vec<String> },
    SealOperation { operation: String, user: String, success: bool },
    ...
    MfaSetup { user: String, method: String },
    MFASuccess { user: String, method: String },
    MFARemoval { user: String, method: String },
    LoginSuccess { user: String, method: String, ip_address: Option<String> },
    Logout { user: String, session_id: String, ip_address: Option<String>, user_agent: Option<String> },
}

#[derive(Debug, Clone)]
pub enum ExportFormat {
    CSV, XML, JSON, CEF, LEEF, SIEM
}
...
impl AuditFilters {
    ...
    pub fn by_user(user: String) -> Self { Self { user: Some(user), ..Default::default() } }
    pub fn by_path(path: String) -> Self { Self { path: Some(path), ..Default::default() } }
}
...
impl AuditLogger {
    ...
    /// Export audit data
    pub async fn export_data(&self, _format: ExportFormat, _filters: AuditFilters) -> Result<Vec<u8>> {
        // Placeholder
        Ok(vec![])
    }
