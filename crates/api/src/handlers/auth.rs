...
mod tests {
    ...
    async fn create_test_server() -> TestServer {
        ...
        // We ignore the result as we only care that the user exists, and failures (e.g. already exists)
        // shouldn't block the test server creation if handled gracefully.
        let _ = services.auth.register_user(
            "alice",
            "password123",
