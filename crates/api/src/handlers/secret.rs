...
pub async fn get_secret(
    ...
    let secret_data: secret::SecretData =
        state
            .secreton
            .get_secret(&path, &user)
            .await
            .map_err(|e| match e {
    ...
    let current_secret = match state.secreton.get_secret(&path, &user).await {
        ...
        Err(e) => {
            return Err(crate::ApiError::Internal(format!(
                "Failed to retrieve current secret: {}",
                e
            )));
        }
    };
...
pub async fn create_backup(
    ...
    Err(crate::ApiError::Internal(
        "Backup functionality not implemented".to_string(),
    ))
}
...
pub async fn delete_backup(
    ...
    state
        .secreton
        .delete_backup(&backup_id, &user)
        .await
        .map_err(|e| match e {
            secret::SecretError::BackupNotFound { .. } => {
                crate::ApiError::NotFound("Backup not found".to_string())
            }
            secret::SecretError::PermissionDenied(msg) => crate::ApiError::Authorization(msg),
