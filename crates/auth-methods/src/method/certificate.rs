...
use crate::error::AuthMethodError;
use crate::model::*;
...
impl CertAuth {
    ...
    pub fn parse_certificate(&self, pem_data: &str) -> Result<CertificateSubject, AuthMethodError> {
        ...
        let der_data = base64.decode(&pem_body)
            .map_err(|_e| AuthMethodError::CertificateParseError(format!("Base64 decode error: {}", _e)))?;
        
        let (_, cert) = X509Certificate::from_der(&der_data)
            .map_err(|_e| AuthMethodError::CertificateParseError(format!("DER parse error: {}", _e)))?;
    ...
    pub async fn validate_certificate(&self, cert: &CertificateSubject) -> Result<(), AuthMethodError> {
        ...
        if now < cert.not_before {
            return Err(AuthMethodError::CertificateNotYetValid);
        }
        if now > cert.not_after {
            return Err(AuthMethodError::CertificateExpired);
        }
        
        // Check revocation
        if revoked.contains(&cert.serial_number) {
            return Err(AuthMethodError::CertificateRevoked);
        }
        
        // Validate against trusted CAs (simplified - in production would verify chain)
        if _config.trusted_cas.is_empty() {
            return Err(AuthMethodError::CertificateUntrustedCa);
        }
    ...
    pub async fn authenticate(&self, pem_data: &str) -> Result<UserInfo, AuthMethodError> {
        // Parse certificate
        ...
        Ok(UserInfo {
            ...
            groups: vec![], // Certificate auth doesn't have groups
            metadata,
            created_at: Utc::now(),
