...
impl CertAuth {
    ...
    pub fn parse_certificate(&self, pem_data: &str) -> Result<CertificateSubject, AuthMethodError> {
        ...
        let der_data = base64.decode(&pem_body).map_err(|_e| {
            AuthMethodError::CertificateParseError(format!("Base64 decode error: {}", _e))
        })?;

        let (_, cert) = X509Certificate::from_der(&der_data).map_err(|_e| {
            AuthMethodError::CertificateParseError(format!("DER parse error: {}", _e))
        })?;
