...
impl AgentService for CombinedAgentService {
    ...
    async fn get_agent(&self, name: String) -> Result<Option<AgentConfig>, AuthMethodError> {
    ...
    async fn update_agent(
        ...
        if let Some(agent) = agents.get_mut(&name) {
            ...
            Err(AuthMethodError::UserNotFound(format!("agent {}", name)))
        }
    }
    ...
    async fn list_agents(&self) -> Result<AgentListResponse, AuthMethodError> {
        let agents = self.agents.read().await;
