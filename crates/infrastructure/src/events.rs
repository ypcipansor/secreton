...
pub struct Event {
    ...
    /// Related entity (_path, token ID, etc.)
...
impl Event {
    ...
    /// Add metadata
    pub fn with_metadata(mut self, _key: String, value: serde_json::Value) -> Self {
...
impl EventService {
    ...
    pub async fn subscribe(&self, subscription: EventSubscription) -> Result<String, EventError> {
        ...
        {
            return Err(EventError::InvalidWebhookUrl(
    ...
    async fn deliver_webhook(&self, event: &Event, subscription: &EventSubscription) {
        // In production, this would make an actual HTTP POST _request
...
mod tests {
    ...
    async fn test_event_filtering() {
        let filter = EventFilter {
            ...
            source_pattern: Some("_secret-*".to_string()),
            entity_pattern: None,
        };

        // Matching event
        let event1 = Event::new(EventType::SecretRead, "_secret-engine".to_string())
            .with_severity(EventSeverity::Warning);
        assert!(filter.matches(&event1));

        // Non-matching event type
        let event2 = Event::new(EventType::AuthSuccess, "_secret-engine".to_string())
            .with_severity(EventSeverity::Warning);
        assert!(!filter.matches(&event2));

        // Non-matching severity
        let event3 = Event::new(EventType::SecretRead, "_secret-engine".to_string())
