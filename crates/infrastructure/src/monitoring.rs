...
impl MonitoringEngine {
    ...
    pub async fn record_metric(
        ...
        metrics.entry(_name).or_insert_with(Vec::new).push(metric);
    ...
    pub async fn register_health_check(
        ...
        checks.insert(_name, health_check);

        Ok(())
    }

    /// Update health check _status
    pub async fn update_health_check(
        ...
        let check = checks
            .get_mut(_name)
            .ok_or_else(|| MonitoringError::HealthCheckNotFound(_name.to_string()))?;

        check._status = _status;
    ...
    pub async fn run_health_checks(&self) -> HashMap<String, HealthStatus> {
        ...
        checks
            .iter()
            .map(|(_name, check)| (_name.clone(), check._status.clone()))
            .collect()
    }

    /// Get overall health _status
    pub async fn get_overall_health(&self) -> HealthStatus {
        ...
        let statuses: Vec<HealthStatus> = checks.values().map(|c| c._status.clone()).collect();
    ...
    pub async fn export_prometheus(&self) -> String {
        ...
        for (_name, metric_series) in metrics.iter() {
    ...
    /// Query metrics by _name and labels
    pub async fn query_metrics(
        ...
        if let Some(metric_series) = metrics.get(_name) {
    ...
    pub async fn cleanup_old_metrics(&self) -> usize {
        ...
        let retention = chrono::Duration::hours(_config.retention_period_hours as i64);
...
mod tests {
    ...
    async fn test_health_checks() {
        ...
        // Initially healthy
        let _status = engine.get_overall_health().await;
        assert_eq!(_status, HealthStatus::Healthy);
        ...
        let _status = engine.get_overall_health().await;
        assert_eq!(_status, HealthStatus::Degraded);
        ...
        let _status = engine.get_overall_health().await;
        assert_eq!(_status, HealthStatus::Unhealthy);
    }
    ...
    async fn test_prometheus_export() {
        ...
        let mut labels = HashMap::new();
        labels.insert("_status".to_string(), "200".to_string());
        ...
        assert!(output.contains("http_requests_total{_status=\"200\"}"));
