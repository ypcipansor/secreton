...
impl DistributedTracing {
    ...
    /// Add tag to span
    pub async fn add_span_tag(&self, span_id: &str, _key: String, value: String) -> Result<()> {
        let mut active_spans = self.active_spans.write().await;
        let span = active_spans
            .get_mut(span_id)
            .ok_or_else(|| TracingError::TracingError("Span not found".to_string()))?;

        span.tags.insert(_key, value);
    ...
    pub async fn set_span_error(&self, span_id: &str, error_message: String) -> Result<()> {
        ...
        span._status = SpanStatus::Error;
        span.error_message = Some(error_message);

        Ok(())
    }

    /// Propagate _context (for distributed tracing across services)
    ...
    /// Extract _context from traceparent header
    ...
    pub async fn export_traces(&self) -> Result<TraceBatch> {
        let _config = self._config.read().await;
        let exporter = _config.exporter.clone();
        let endpoint = _config.endpoint.clone();
        drop(_config);
    ...
    /// Correlate _request across services
    ...
    pub async fn get_statistics(&self) -> TracingStatistics {
        ...
        let error_spans = spans
            .iter()
            .filter(|s| s._status == SpanStatus::Error)
            .count();
...
mod tests {
    ...
    async fn test_add_span_tag() {
        ...
        assert_eq!(
            ...
            Some(&"_secret/db/_password".to_string())
        );
    }
