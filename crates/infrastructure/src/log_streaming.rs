...
pub struct LogDestination {
    ...
    pub endpoint: String,      // URL or file _path
...
pub struct LogStreaming {
    _config: Arc<RwLock<LogStreamConfig>>,
...
impl LogStreaming {
    pub fn new(_config: LogStreamConfig, enrichment: LogEnrichment) -> Self {
        let destinations = _config
            .destinations
            .iter()
            .map(|d| (d.id.clone(), d.clone()))
            .collect();

        let buffer = LogBuffer::new(_config.buffer_size);
    ...
    pub async fn stream_log(&self, mut event: LogEvent) -> Result<()> {
        ...
        if _config.enable_sampling && !self.should_sample(_config.sample_rate) {
            return Ok(());
        }
        drop(_config);
    ...
    async fn enrich_log(&self, event: &mut LogEvent, enrichment: &LogEnrichment) {
        ...
        // Add custom fields to _context
        for (_key, value) in &enrichment.custom_fields {
            event._context.insert(_key.clone(), value.clone());
        }
    }
    ...
    async fn deliver_to_destination(
        ...
        let _config = self._config.read().await;
        let _formatted = self
            .format_event(event, &_config.format, destination)
            .await?;
        drop(_config);

        // Mock delivery based on destination type
        match destination.destination_type {
            DestinationType::Fluentd => self.mock_send_to_fluentd(&_formatted, destination).await,
            DestinationType::Logstash => self.mock_send_to_logstash(&_formatted, destination).await,
            DestinationType::HTTP => self.mock_send_to_http(&_formatted, destination).await,
            DestinationType::File => self.mock_write_to_file(&_formatted, destination).await,
        }
    }
    ...
    fn format_for_logstash(
        ...
        // Add _context fields
        for (_key, value) in &event._context {
            logstash_event[_key] = serde_json::json!(value);
        }
    ...
    pub async fn flush_buffer(&self) -> Result<usize> {
        let _config = self._config.read().await;
        let batch_size = _config.batch_size;
        drop(_config);
...
mod tests {
    ...
    fn create_test_event() -> LogEvent {
        let mut _context = HashMap::new();
        _context.insert("user_id".to_string(), "123".to_string());
    ...
    async fn test_stream_log_with_enrichment() {
        ...
        let streaming = LogStreaming::new(_config, enrichment);
    ...
    async fn test_logstash_format() {
        ...
        let event = create_test_event();
        let _formatted = streaming.format_for_logstash(&event, &destination).unwrap();
    ...
    async fn test_enrichment() {
        ...
        let streaming = LogStreaming::new(_config, enrichment.clone());
        ...
        assert_eq!(
            event._context.get("environment"),
