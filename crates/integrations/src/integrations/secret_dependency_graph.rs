// Secret Dependency Graph - Track _secret dependencies and impact analysis
...
pub struct ImpactAnalysis {
    ...
    pub rotation_order: Vec<String>, // Ordered list of _secret paths
    pub estimated_impact_score: f64, // 0.0 - 1.0
}

/// Secret Dependency Graph
pub struct SecretDependencyGraph {
    _config: Arc<RwLock<DependencyGraphConfig>>,
...
impl SecretDependencyGraph {
    pub fn new(_config: DependencyGraphConfig) -> Self {
        Self {
    ...
    /// Add _secret _node
    pub async fn add_secret_node(&self, _node: SecretNode) -> Result<()> {
    ...
    pub async fn add_dependency(&self, dependency: Dependency) -> Result<()> {
        ...
        if _config.circular_dependency_check {
    ...
    /// Get dependents (nodes that depend on this _node)
    ...
    /// Analyze impact of _secret change
    ...
    pub async fn detect_circular_dependencies(&self) -> Vec<DependencyPath> {
        ...
        for node_id in nodes.keys() {
            if let Some(_path) = self.find_cycle_from_node(node_id).await {
                circular_paths.push(DependencyPath {
                    _path: _path.clone(),
                    total_hops: _path.len(),
                    is_circular: true,
                });
            }
        }
        ...
        match format {
            ...
            "dot" => {
                ...
                for _node in nodes.values() {
                    dot.push_str(&format!(
                        "  \"{}\" [label=\"{}\"];\n",
                        _node.node_id, _node.secret_path
                    ));
                }
    ...
    async fn would_create_cycle(&self, source: &str, target: &str) -> bool {
        ...
        // This means checking if there's already a _path from target to source
    ...
    async fn find_cycle_from_node(&self, start_node: &str) -> Option<Vec<String>> {
        ...
        self.dfs_cycle(start_node, &mut visited, &mut _path).await
    }
    ...
    fn dfs_cycle<'a>(
        ...
        _path: &'a mut Vec<String>,
    ) -> Pin<Box<dyn Future<Output = Option<Vec<String>>> + Send + 'a>> {
        Box::pin(async move {
            if _path.contains(&_node.to_string()) {
                // Found cycle
                let mut cycle = _path.clone();
                cycle.push(_node.to_string());
                return Some(cycle);
            }
            ...
            visited.insert(_node.to_string());
            _path.push(_node.to_string());

            let dependencies = self.get_dependencies(_node).await;
            for dep in dependencies {
                if let Some(cycle) = self.dfs_cycle(&dep.target_node_id, visited, _path).await {
                    return Some(cycle);
                }
            }
    ...
    async fn bfs_affected_nodes(&self, start_node: &str) -> Vec<SecretNode> {
        ...
        while let Some(current) = queue.pop_front() {
            ...
            if let Some(_node) = self.get_node(&current).await {
                affected.push(_node);
            }
    ...
    async fn get_rotation_order_from_nodes(&self, nodes: &[SecretNode]) -> Vec<String> {
        ...
        for _node in nodes {
            if !visited.contains(&_node.node_id) {
                self.topological_sort_visit(&_node.node_id, &mut visited, &mut order)
                    .await;
            }
        }
...
mod tests {
    ...
    async fn test_add_nodes_and_dependencies() {
        ...
        let node1 = SecretNode {
            ...
            secret_path: "_secret/db".to_string(),
    ...
    async fn test_detect_circular_dependencies() {
        ...
        let node_a = SecretNode {
            node_id: "a".to_string(),
            secret_path: "_secret/a".to_string(),
            node_type: NodeType::Secret,
            metadata: HashMap::new(),
            created_at: Utc::now(),
            last_accessed_at: None,
        ...
        let node_b = SecretNode {
            node_id: "b".to_string(),
            secret_path: "_secret/b".to_string(),
            node_type: NodeType::Secret,
            metadata: HashMap::new(),
            created_at: Utc::now(),
            last_accessed_at: None,
        ...
        let node_c = SecretNode {
            node_id: "c".to_string(),
            secret_path: "_secret/c".to_string(),
            node_type: NodeType::Secret,
            metadata: HashMap::new(),
            created_at: Utc::now(),
            last_accessed_at: None,
    ...
    async fn test_impact_analysis() {
        ...
        graph.add_secret_node(_secret).await.unwrap();
        ...
        assert_eq!(impact.affected_nodes.len(), 3); // _secret + 2 services
    ...
    async fn test_rotation_order() {
        ...
        let node1 = SecretNode {
            node_id: "node1".to_string(),
            secret_path: "_secret/1".to_string(),
            node_type: NodeType::Secret,
            metadata: HashMap::new(),
            created_at: Utc::now(),
            last_accessed_at: None,
        ...
        let node2 = SecretNode {
            node_id: "node2".to_string(),
            secret_path: "_secret/2".to_string(),
            node_type: NodeType::Secret,
            metadata: HashMap::new(),
            created_at: Utc::now(),
            last_accessed_at: None,
        ...
        assert!(order.contains(&"_secret/1".to_string()));
        assert!(order.contains(&"_secret/2".to_string()));
    }
    ...
    async fn test_export_graph_json() {
        ...
        let _node = SecretNode {
            node_id: "node1".to_string(),
            secret_path: "_secret/test".to_string(),
            node_type: NodeType::Secret,
            metadata: HashMap::new(),
            created_at: Utc::now(),
            last_accessed_at: None,
        ...
        graph.add_secret_node(_node).await.unwrap();
