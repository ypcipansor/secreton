...
pub struct AdvancedBackupRecovery {
    _config: Arc<RwLock<BackupConfig>>,
...
impl AdvancedBackupRecovery {
    ...
    pub async fn create_backup(&self, backup_type: BackupType) -> Result<Backup> {
        ...
        let compression_enabled = _config.compression_enabled;
        let encryption_enabled = _config.encryption_enabled;
        drop(_config);
        ...
        let _data = serde_json::to_string(&backup_metadata).unwrap();
        let compressed_size = if compression_enabled {
            (_data.len() as f64 * 0.4) as u64 // Simulate 60% compression
        } else {
            _data.len() as u64
        };
        ...
        // Update last _snapshot for incremental backups
    ...
    pub async fn create_incremental_backup(&self) -> Result<Backup> {
        ...
        let changed_secrets: HashMap<_, _> = current_secrets
            ...
            .filter(|(_path, _data)| {
                last_secrets
                    .get(_path.as_str())
                    .map(|old| old.version != _data.version)
                    .unwrap_or(true)
            })
        ...
        let backup_id = uuid::Uuid::new_v4().to_string();
        let _data = serde_json::to_string(&changed_secrets).unwrap();
        let checksum = self.calculate_checksum(&_data);

        let _config = self._config.read().await;
        let compressed = _config.compression_enabled;
        let encrypted = _config.encryption_enabled;
        drop(_config);
        ...
        let backup = Backup {
            ...
            size_bytes: (_data.len() as f64 * 0.3) as u64, // Smaller for incremental
            compressed,
            encrypted,
            created_at: Utc::now(),
            metadata: HashMap::from([
        ...
        backups
            ...
            .filter(|b| {
                let type_match = backup_type
                    ...
                    .unwrap_or(true);
                let status_match = _status.as_ref().map(|s| &b._status == s).unwrap_or(true);
    ...
    pub async fn cleanup_old_backups(&self) -> Result<usize> {
        let _config = self._config.read().await;
        let retention_count = _config.retention_count as usize;
        drop(_config);
    ...
    async fn mock_fetch_secrets(&self) -> Result<HashMap<String, SecretData>> {
        ...
        *call_count += 1;
        ...
        secrets.insert(
            ...
            SecretData {
                _path: "_secret/db/_password".to_string(),
        ...
        secrets.insert(
            ...
            SecretData {
                _path: "_secret/api/_key".to_string(),
    ...
    fn calculate_checksum(&self, _data: &str) -> String {
...
mod tests {
    ...
    async fn test_create_full_backup() {
        ...
        assert_eq!(backup._status, BackupStatus::Completed);
    ...
    async fn test_restore_from_backup() {
        ...
        assert_eq!(restore_op._status, BackupStatus::Completed);
