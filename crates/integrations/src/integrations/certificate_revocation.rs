...
impl CertificateRevocationService {
    ...
    pub async fn configure_crl(&self, _config: CRLConfig) -> Result<()> {
        let mut configs = self.crl_config.write().await;
        configs.insert(_config.issuer.clone(), _config);
        Ok(())
    }

    /// Configure OCSP responder
    pub async fn configure_ocsp(&self, _config: OCSPConfig) -> Result<()> {
        let mut ocsp_config = self.ocsp_config.write().await;
        *ocsp_config = Some(_config);
        Ok(())
    }
    ...
    pub async fn rebuild_crl(&self, issuer: &str) -> Result<CRL> {
        ...
        crl.next_update = Utc::now() + Duration::days(_config.validity_period_days as i64);

        if !_config.distribution_points.is_empty() {
            crl.distribution_point = Some(_config.distribution_points[0].clone());
        }
    ...
    pub async fn process_ocsp_request(
        ...
        let _config = ocsp_config
            .as_ref()
            .ok_or_else(|| RevocationError::OCSPError("OCSP not configured".to_string()))?;

        let responder_id = _config.responder_id.clone();
        drop(ocsp_config);

        // Create _request
        let _request = OCSPRequest {
            id: uuid::Uuid::new_v4().to_string(),
...
mod tests {
    ...
    async fn test_crl_generation() {
        ...
        service.configure_crl(_config).await.unwrap();
    ...
    async fn test_ocsp_request() {
        ...
        assert_eq!(response._status, OCSPStatus::Good);
    }
