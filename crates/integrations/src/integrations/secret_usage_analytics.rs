...
pub struct SecretUsageAnalytics {
    _config: Arc<RwLock<AnalyticsConfig>>,
...
impl SecretUsageAnalytics {
    ...
    /// Get usage metrics for a _secret
        ...
        match format {
            "json" => {
                let json = serde_json::to_string_pretty(&*access_events)
                    .map_err(|_e| AnalyticsError::ExportError(_e.to_string()))?;
                Ok(json)
            }
            "csv" => {
    ...
    pub async fn clear_old_events(&self) -> Result<usize> {
        ...
        access_events.retain(|_e| _e.timestamp >= threshold);
