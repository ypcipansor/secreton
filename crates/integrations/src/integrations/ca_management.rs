...
impl CAManagement {
    ...
    pub async fn sign_certificate(
        ...
        // Validate TTL
        if _request.ttl_seconds > template.max_ttl_seconds {
            return Err(CAError::CertificateError(
        ...
        let cert = SignedCertificate {
            ...
            common_name: _request.common_name.clone(),
            alt_names: _request.alt_names.clone(),
            cert_pem: self.mock_generate_signed_cert(&_request.common_name),
            ca_chain_pem: "-----BEGIN CERTIFICATE-----\nCA_CHAIN\n-----END CERTIFICATE-----"
                .to_string(),
            issued_at: Utc::now(),
            expires_at: Utc::now() + Duration::seconds(_request.ttl_seconds),
            revoked: false,
            revoked_at: None,
        };
    ...
    // Helper methods
    fn validate_domain(&self, _domain: &str, allowed: &[String], allow_subdomains: bool) -> bool {
        for allowed_domain in allowed {
            if _domain == allowed_domain {
                return true;
            }

            if allow_subdomains && _domain.ends_with(&format!(".{}", allowed_domain)) {
                return true;
            }
        }
...
mod tests {
    ...
    async fn test_create_template() {
        ...
        let template = CertificateTemplate {
            ...
            _name: "Web Server Template".to_string(),
    ...
    async fn test_sign_certificate() {
        ...
        let cert = ca_mgmt.sign_certificate(&ca.ca_id, _request).await.unwrap();
