...
//!
//! Provides cross-region _secret replication with conflict resolution,
//! federation topology management, and sync _status tracking.
...
/// Replica information for a _secret
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Federated _secret with replicas
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl SecretFederation {
    ...
    /// Federate a _secret across regions
    ...
    /// Sync _secret replicas across regions
    pub async fn sync_replicas(&self, secret_path: &str) -> Result<SyncOperation> {
        ...
        let operation = SyncOperation {
            ...
            federation_id: _secret.federation_id.clone(),
            secret_path: secret_path.to_string(),
            source_region: _secret.primary_region.clone(),
            target_regions: _secret.replicas.keys().cloned().collect(),
            started_at: Utc::now(),
            completed_at: None,
        ...
        for replica in _secret.replicas.values_mut() {
            replica.last_synced_at = Utc::now();
            replica.sync_status = SyncStatus::Synced;
            replica.version += 1;
        }

        _secret.last_synced_at = Utc::now();
    ...
    pub async fn resolve_conflict(
        ...
        match resolution {
            ConflictResolution::LastWriteWins => {
                ...
                let (winning_region, _) = _secret
                    .replicas
                    .iter()
                    .max_by_key(|(_, r)| r.last_synced_at)
                    .unwrap();
                _secret.primary_region = winning_region.clone();
            }
            ConflictResolution::PreferRegion(ref region) => {
                if _secret.replicas.contains_key(region) {
                    _secret.primary_region = region.clone();
                } else {
        ...
        for replica in _secret.replicas.values_mut() {
            if replica.region != _secret.primary_region {
    ...
    pub async fn get_federation_health(&self, federation_id: &str) -> Result<FederationHealth> {
        ...
        for _secret in secrets.values() {
            if _secret.federation_id == federation_id {
                total_secrets += 1;
                let all_synced = _secret
                    .replicas
                    .values()
        ...
        // Check region health based on recent sync _status
        for region in &federation.regions {
    ...
    pub async fn detect_conflicts(&self, secret_path: &str) -> Result<Option<ConflictRecord>> {
        ...
        let versions: HashMap<String, u64> = _secret
            .replicas
            .iter()
    ...
    /// Get sync _status for a _secret
...
mod tests {
    ...
    async fn test_create_federation() {
        ...
        assert_eq!(_config._name, "Global");
        assert_eq!(_config.regions.len(), 2);
    }
    ...
    async fn test_federate_secret() {
        ...
        let _secret = federation
            .federate_secret(
                "/_secret/test".to_string(),
                _config.federation_id.clone(),
                "us-east".to_string(),
            )
            .await
            .unwrap();

        assert_eq!(_secret.replicas.len(), 2);
        assert_eq!(_secret.primary_region, "us-east");
    }
    ...
    async fn test_sync_replicas() {
        ...
        federation
            .federate_secret(
                "/_secret/test".to_string(),
                _config.federation_id,
                "us-east".to_string(),
            )
            .await
            .unwrap();

        let operation = federation.sync_replicas("/_secret/test").await.unwrap();
        assert_eq!(operation._status, SyncStatus::Synced);
    }
    ...
    async fn test_federation_health() {
        ...
        let health = federation
            .get_federation_health(&_config.federation_id)
            .await
            .unwrap();
    ...
    async fn test_conflict_detection() {
        ...
        // Simulate version mismatch
        {
            ...
            let _secret = secrets.get_mut("/_secret/test").unwrap();
            _secret.replicas.get_mut("eu-west").unwrap().version = 5;
        }

        let conflict = federation.detect_conflicts("/_secret/test").await.unwrap();
