...
//!
//! Provides automatic _secret scanning in code/configs, _secret classification
...
impl SecretDiscovery {
    ...
    pub async fn scan_repository(&self, _config: ScanConfig) -> Result<ScanResult> {
        let scan_id = _config.scan_id.clone();
        ...
        for rule in rules.values().filter(|r| r.enabled) {
            if self.should_scan_with_rule(rule, &_config) {
                let discovery = self.create_mock_discovery(rule, &_config).await;
        ...
        result._status = ScanStatus::Completed;
    ...
    fn should_scan_with_rule(&self, rule: &DiscoveryRule, _config: &ScanConfig) -> bool {
        if _config.rules.is_empty() {
            return true;
        }
        _config.rules.contains(&rule.rule_id)
    }
    ...
    async fn create_mock_discovery(
        ...
        let target_path = match &_config.target {
            ScanTarget::Repository(_path) => _path.clone(),
            ScanTarget::Directory(_path) => _path.clone(),
            ScanTarget::File(_path) => _path.clone(),
        };
        ...
        DiscoveredSecret {
            ...
            location: SecretLocation {
                ...
                file_path: format!("{}/_config.yaml", target_path),
                line_number: 42,
                _context: "_password: ***REDACTED***".to_string(),
            },
    ...
    pub async fn enrich_metadata(
        ...
        if let Some(discovery) = discoveries.get_mut(discovery_id) {
            ...
            for (_key, value) in enrichment.custom_fields {
                discovery.metadata.insert(_key, value);
            }
        }
    ...
    /// Update remediation _status
    pub async fn update_remediation_status(
        &self,
        discovery_id: &str,
        _status: RemediationStatus,
    ) -> Result<()> {
        let mut discoveries = self.discoveries.write().await;

        if let Some(discovery) = discoveries.get_mut(discovery_id) {
            discovery.remediation_status = _status;
        }
...
mod tests {
    ...
    async fn test_scan_repository() {
        ...
        let rule = DiscoveryRule {
            ...
            pattern: r"_password\s*=\s*.+".to_string(),
        ...
        let _config = ScanConfig {
            ...
            target: ScanTarget::Repository("/repo/_path".to_string()),
            rules: vec![],
            recursive: true,
            max_file_size_mb: 10,
        ...
        let result = discovery.scan_repository(_config).await.unwrap();

        assert_eq!(result._status, ScanStatus::Completed);
    ...
    async fn test_get_discoveries_by_sensitivity() {
        ...
        discovery.scan_repository(_config).await.unwrap();
