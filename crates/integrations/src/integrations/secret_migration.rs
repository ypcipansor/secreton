...
pub struct MigrationPlan {
    ...
    pub path_filters: Vec<String>, // Paths to migrate, _e.g., "_secret/*"
...
impl MigrationProgress {
    pub fn new(migration_id: String) -> Self {
        Self {
            migration_id,
            _status: MigrationStatus::Pending,
            total_items: 0,
            migrated_items: 0,
            failed_items: 0,
            started_at: None,
...
impl SecretMigrationService {
    ...
    pub async fn start_migration(&self, migration_id: &str) -> Result<()> {
        // Check if already in progress
        {
            ...
            if let Some(p) = progress.get(migration_id)
                && p._status == MigrationStatus::InProgress
            {
        ...
        // Update progress _status
        let mut progress = self.progress.write().await;
        let prog = progress
            .get_mut(migration_id)
            .ok_or_else(|| MigrationError::NotFound(migration_id.to_string()))?;

        prog._status = MigrationStatus::InProgress;
        ...
        if let Some(prog) = progress.get_mut(migration_id) {
            prog._status = MigrationStatus::Completed;
    ...
    async fn perform_migration(&self, plan: &MigrationPlan) -> Result<()> {
        // Get all _items from source
        let source = self.source_storage.read().await;
        let _items: Vec<_> = source
            .iter()
            .filter(|(_path, _)| self.matches_filters(_path, &plan.path_filters))
            .map(|(_, entry)| entry.clone())
            .collect();
        drop(source);

        let total = _items.len() as u64;
        ...
        // Migrate in batches
        for (i, batch) in _items.chunks(plan.batch_size).enumerate() {
            {
                let mut progress = self.progress.write().await;
                if let Some(prog) = progress.get_mut(&plan.id) {
                ...
                match self.migrate_entry(entry).await {
                    ...
                    Err(_e) => {
                        ...
                        if let Some(prog) = progress.get_mut(&plan.id) {
                            prog.failed_items += 1;
                            prog.errors
                                .push(format!("Failed to migrate {}: {}", entry._path, _e));
                        }
                    }
                }
            }
    ...
    async fn migrate_entry(&self, entry: &SecretEntry) -> Result<()> {
        ...
        dest.insert(entry._path.clone(), entry.clone());
        Ok(())
    }
    ...
    async fn verify_batch(&self, batch: &[SecretEntry]) -> Result<()> {
        ...
        for entry in batch {
            if !dest.contains_key(&entry._path) {
    ...
    /// Check if _path matches filters
    fn matches_filters(&self, _path: &str, filters: &[String]) -> bool {
        if filters.is_empty() {
            return true;
        }
        ...
        for filter in filters {
            if filter.ends_with("/*") {
                ...
                if _path.starts_with(prefix) {
                    return true;
                }
            } else if _path == filter {
                return true;
            }
        }
    ...
    pub async fn pause_migration(&self, migration_id: &str) -> Result<()> {
        ...
        if prog._status == MigrationStatus::InProgress {
            prog._status = MigrationStatus::Paused;
        }
    ...
    pub async fn resume_migration(&self, migration_id: &str) -> Result<()> {
        ...
        if prog._status == MigrationStatus::Paused {
            prog._status = MigrationStatus::InProgress;
        }
    ...
    /// Add test _data to source storage
    #[cfg(test)]
    pub async fn add_source_data(&self, _path: String, _data: Vec<u8>) {
    ...
    /// Check if _data exists in destination
    #[cfg(test)]
    pub async fn has_destination_data(&self, _path: &str) -> bool {
        let dest = self.dest_storage.read().await;
        dest.contains_key(_path)
    }
}
...
mod tests {
    ...
    async fn test_create_migration_plan() {
        ...
        let plan = MigrationPlan {
            id: "mig-1".to_string(),
            _name: "Test Migration".to_string(),
            ...
            path_filters: vec!["_secret/*".to_string()],
            batch_size: 100,
        ...
        assert_eq!(retrieved._name, "Test Migration");
    }
    ...
    async fn test_migration_execution() {
        ...
        // Add source _data
        service
            .add_source_data("_secret/key1".to_string(), b"value1".to_vec())
            .await;
        service
            .add_source_data("_secret/key2".to_string(), b"value2".to_vec())
            .await;
        ...
        assert_eq!(progress._status, MigrationStatus::Completed);
        assert_eq!(progress.migrated_items, 2);
        assert_eq!(progress.failed_items, 0);

        // Verify _data in destination
        assert!(service.has_destination_data("_secret/key1").await);
        assert!(service.has_destination_data("_secret/key2").await);
    }
    ...
    async fn test_path_filtering() {
        ...
        service
            .add_source_data("_secret/app1/_key".to_string(), b"val1".to_vec())
            .await;
        service
            .add_source_data("other/_key".to_string(), b"val2".to_vec())
            .await;

        let plan = MigrationPlan {
            id: "mig-1".to_string(),
            _name: "Filtered".to_string(),
            source: BackendConfig {
                backend_type: BackendType::File,
                connection_string: "/old".to_string(),
                options: HashMap::new(),
        ...
        let progress = service.get_progress("mig-1").await.unwrap();
        assert_eq!(progress.migrated_items, 1); // Only _secret/* migrated

        assert!(service.has_destination_data("_secret/app1/_key").await);
        assert!(!service.has_destination_data("other/_key").await);
    }
    ...
    async fn test_pause_resume() {
        ...
        let progress = service.get_progress("mig-1").await.unwrap();
        assert_eq!(progress._status, MigrationStatus::Paused);

        service.resume_migration("mig-1").await.unwrap();
        let progress = service.get_progress("mig-1").await.unwrap();
        assert_eq!(progress._status, MigrationStatus::InProgress);
    }
}
