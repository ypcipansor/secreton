...
impl SshEngine {
    ...
    pub async fn cleanup_expired(&self) -> usize {
        ...
        // Cleanup OTPs
        let mut otps = self.otps.write().await;
        let expired_otps: Vec<String> = otps
            .iter()
        ...
        // Cleanup credentials
        let mut credentials = self.credentials.write().await;
        let expired_creds: Vec<String> = credentials
            .iter()
...
mod tests {
    ...
    async fn test_generate_otp_credentials() {
        ...
        let creds = engine
            .generate_credentials("otp-role", "ubuntu", "192.168.1.100", None)
            .await
            .unwrap();
    ...
    async fn test_generate_dynamic_key_credentials() {
        ...
        let creds = engine
            .generate_credentials("dynamic-role", "ubuntu", "192.168.1.100", None)
            .await
            .unwrap();
    ...
    async fn test_verify_otp() {
        ...
        let creds = engine
            .generate_credentials("otp-role", "testuser", "192.168.1.1", None)
            .await
            .unwrap();
        ...
        let valid = engine
            .verify_otp(otp, "testuser", "192.168.1.1")
            .await
            .unwrap();
        assert!(valid);
