...
pub enum StandbyError {
    #[error("Standby _node not found: {0}")]
    NotFound(String),
...
/// Standby _node information
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct StandbyNode {
    pub node_id: String,
    pub _config: StandbyConfig,
...
impl StandbyNode {
    pub fn new(_config: StandbyConfig) -> Self {
        Self {
    ...
    pub fn is_sync_lagging(&self) -> bool {
        if let Some(last_sync) = self.last_sync {
            ...
            elapsed.num_seconds() > self._config.max_sync_lag_seconds as i64
        } else {
            true // Never synced
        }
    }
    ...
    pub fn update_sync_success(&mut self, _items: u64, _bytes: u64, duration_ms: u64) {
        ...
        self.next_sync =
            Some(Utc::now() + chrono::Duration::seconds(self._config.sync_interval_seconds as i64));
        self.sync_count += 1;
...
impl PerformanceStandbyService {
    ...
    /// Register a new standby _node
    pub async fn register_standby(&self, _config: StandbyConfig) -> Result<StandbyNode> {
        let node_id = _config.node_id.clone();
        let standby = StandbyNode::new(_config);
    ...
    /// Start standby mode for a _node
    ...
    pub async fn sync_from_primary(&self, node_id: &str) -> Result<SyncResult> {
        ...
        let (success, _items, _bytes) = self.simulate_sync(&standby._config.primary_address).await;

        let duration_ms = (Utc::now() - start_time).num_milliseconds() as u64;

        let result = if success {
            standby.update_sync_success(_items, _bytes, duration_ms);
    ...
    /// Read _data from standby (with caching)
    pub async fn read(&self, node_id: &str, _key: &str) -> Result<Option<Vec<u8>>> {
        let start_time = Utc::now();
        ...
        let enable_caching = standby._config.enable_caching;
        drop(nodes);
        ...
        let result = if enable_caching {
            ...
            if let Some(entry) = cache.get(_key) {
                if !entry.is_expired() {
                    cache_hit = true;
    ...
    pub async fn cache_write(&self, _key: String, value: Vec<u8>, ttl_seconds: u64) -> Result<()> {
    ...
    pub async fn invalidate_cache(&self, _key: &str) -> Result<()> {
        let mut cache = self.cache.write().await;
        cache.remove(_key);
        Ok(())
    }
    ...
    pub async fn get_sync_history(&self, node_id: Option<&str>, limit: usize) -> Vec<SyncResult> {
        ...
        history
            .iter()
            .filter(|h| node_id.is_none_or(|id| h.node_id == id))
            .rev()
            .take(limit)
            .cloned()
            .collect()
    ...
    async fn simulate_sync(&self, _primary_address: &str) -> (bool, u64, u64) {
        // In production, would make HTTP _request to primary
