    ...
    C: CacheBackend,
{
    async fn store(&self, entry: &VaultEntry) -> StorageResult<()> {
        ...
        if result.is_ok() {
            ...
            let _ = self.cache.set(&Self::cache_key_for_id(entry.id), serialized.clone(), Some(self.default_ttl)).await;
            let _ = self.cache.set(&Self::cache_key_for_path(&entry.path), serialized, Some(self.default_ttl)).await;
        }
