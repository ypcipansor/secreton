...
impl MongoDBStorage {
    ...
    fn vault_entry_to_mongodb_doc(
        ...
        doc.insert(
            ...
            serde_json::Value::String(general_purpose::STANDARD.encode(&entry.encrypted_data)),
        );
        doc.insert(
            "encryption_metadata".to_string(),
            serde_json::to_value(&entry.encryption_metadata).unwrap_or_default(),
        );
        doc.insert(
            "security_level".to_string(),
            serde_json::Value::Number((entry.security_level as i16).into()),
        );
        doc.insert(
            "metadata".to_string(),
        ...
        doc.insert(
            "version".to_string(),
            serde_json::Value::Number(entry.version.into()),
        );
        doc.insert(
            "owner_id".to_string(),
            serde_json::Value::String(entry.owner_id.to_string()),
        );
        doc.insert(
            "created_at".to_string(),
            serde_json::Value::String(entry.created_at.to_rfc3339()),
        );
        doc.insert(
            "updated_at".to_string(),
            serde_json::Value::String(entry.updated_at.to_rfc3339()),
        );
    ...
    fn mongodb_doc_to_vault_entry(
        &self,
        doc: serde_json::Map<String, serde_json::Value>,
    ) -> StorageResult<VaultEntry> {
        let id = Uuid::parse_str(doc.get("id").and_then(|v| v.as_str()).ok_or_else(|| {
        ...
        let encryption_metadata: crate::EncryptionMetadata =
            serde_json::from_value(doc.get("encryption_metadata").cloned().unwrap_or_default())
                .map_err(|e| StorageError::SerializationError {
                    message: format!("Invalid encryption metadata: {}", e),
                })?;

        let security_level_int: i16 = doc
            .get("security_level")
            .and_then(|v| v.as_i64())
            .ok_or_else(|| StorageError::SerializationError {
        ...
        let version: u32 = doc.get("version").and_then(|v| v.as_u64()).ok_or_else(|| {
            StorageError::SerializationError {
                message: "Missing version field".to_string(),
            }
        })? as u32;

        let owner_id = Uuid::parse_str(doc.get("owner_id").and_then(|v| v.as_str()).ok_or_else(
            || StorageError::SerializationError {
