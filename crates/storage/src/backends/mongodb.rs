...
impl StorageBackend for MongoDBStorage {
    ...
    async fn get_by_id(&self, id: Uuid) -> StorageResult<Option<VaultEntry>> {
        ...
        if let Some(doc) = result {
            let data: String = doc.get_str("data").map_err(|e| StorageError::SerializationError {
                message: format!("Failed to get data field: {}", e),
            })?.to_string();

            let mut entry: VaultEntry = serde_json::from_str(&data).map_err(|e| {
            ...
            if let Ok(created_at) = doc.get_i64("created_at") {
                entry.created_at = chrono::DateTime::from_timestamp_millis(created_at).unwrap_or(entry.created_at);
            }
            if let Ok(updated_at) = doc.get_i64("updated_at") {
                entry.updated_at = chrono::DateTime::from_timestamp_millis(updated_at).unwrap_or(entry.updated_at);
            }
    ...
    async fn list(&self, params: &QueryParams) -> StorageResult<Vec<VaultEntry>> {
        ...
        if let Some(prefix) = &params.path_prefix {
            filter.insert("path", doc! { "$regex": format!("^{}", regex::escape(prefix)) });
        }

        // Add security level filter
        if let Some(min_level) = params.security_level {
            let level_values = match min_level {
                SecurityLevel::Public => vec!["public", "internal", "confidential", "secret", "top_secret"],
        ...
        if !params.include_expired {
            ...
            filter.insert("expires_at", doc! { "$or": vec![doc! { "$exists": false }, doc! { "$gt": now }] });
        }
        ...
        while let Some(result) = cursor.try_next().await.map_err(|e| StorageError::BackendError {
            backend: "mongodb".to_string(),
            message: format!("Failed to iterate results: {}", e),
        })? {
            let data: String = result.get_str("data").map_err(|e| StorageError::SerializationError {
                message: format!("Failed to get data field: {}", e),
            })?.to_string();
    ...
    async fn get_stats(&self) -> StorageResult<StorageStats> {
        ...
        let pipeline = vec![
            doc! { "$group": { "_id": "$security_level", "count": { "$sum": 1 } } }
        ];
        ...
        entries_created_today = self.collection
            .count_documents(filter, None)
            .await
            .map_err(|e| StorageError::BackendError {
                backend: "mongodb".to_string(),
        ...
        entries_updated_today = self.collection
            .count_documents(filter, None)
            .await
            .map_err(|e| StorageError::BackendError {
                backend: "mongodb".to_string(),
        ...
        expired_entries = self.collection
            .count_documents(filter, None)
            .await
            .map_err(|e| StorageError::BackendError {
                backend: "mongodb".to_string(),
        ...
        if let Some(result) = cursor.try_next().await.map_err(|e| StorageError::BackendError {
            backend: "mongodb".to_string(),
