//! Unified error type shared by every Secreton crate.
//!
//! [`SecretonError`] carries the HTTP semantics of a failure ([`SecretonError::status_code`],
//! [`SecretonError::category`]) but deliberately does **not** know how to turn itself into a
//! response body. The `IntoResponse` implementation lives in `secreton-server`, so this crate
//! stays free of any web framework and keeps compiling for `wasm32-unknown-unknown`.
