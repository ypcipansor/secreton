...
pub trait HardwareService: Send + Sync {
    ...
    ) -> AuthMethodResult<HardwareEnrollment>;

    /// Authenticate with a hardware token
    async fn authenticate(&self, request: HardwareAuthenticationRequest) -> AuthMethodResult<bool>;

    /// Get enrollment for an entity
    async fn get_enrollment(&self, entity_id: Uuid)
    -> AuthMethodResult<Option<HardwareEnrollment>>;

    /// List all enrollments for an entity
    async fn list_enrollments(&self, entity_id: Uuid) -> AuthMethodResult<Vec<HardwareEnrollment>>;
