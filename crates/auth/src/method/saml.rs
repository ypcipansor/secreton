    ...
    pub fn generate_authn_request(&self) -> AuthMethodResult<String> {
        let config = self
            ...
            .ok_or_else(|| SecretonError::Configuration {
                message: "SAML config not set".to_string(),
            })?;
    ...
    <saml:Issuer>{}</saml:Issuer>
        ...
        encoder
            ...
            .map_err(|e| SecretonError::Internal {
                message: format!("SAML compression failed: {e}"),
            })?;
        let compressed = encoder.finish().map_err(|e| SecretonError::Internal {
            message: format!("SAML compression finalize failed: {e}"),
        })?;
    ...
    pub fn parse_saml_response(&self, saml_response: &str) -> AuthMethodResult<SamlAssertion> {
        ...
        let xml_data = if decoded.len() > 2 && decoded[0] == 0x1f && decoded[1] == 0x8b {
            ...
            decoder
                ...
                .map_err(|e| SecretonError::Parse {
                    message: format!("SAML decompression failed: {e}"),
                })?;
            decompressed
        } else {
            decoded
        ...
        let xml_str = String::from_utf8(xml_data).map_err(|e| SecretonError::Parse {
            message: format!("SAML UTF-8 decode failed: {e}"),
        })?;
    ...
    fn parse_saml_xml(&self, xml: &str) -> AuthMethodResult<SamlAssertion> {
        ...
        for event in parser {
            match event {
                ...
                Err(e) => {
                    return Err(SecretonError::Parse {
                        message: format!("SAML XML parsing error: {e}"),
                    });
                }
                _ => {}
            }
