...
impl AwsAuthMethod {
    ...
    async fn validate_credentials(
        ...
        let identity = sts_client
            ...
            .await
            .map_err(|e| {
                SecretonError::AwsError(format!("Failed to validate credentials: {}", e))
            })?;

        let account = identity.account().ok_or(SecretonError::AwsError(
            "No account in identity".to_string(),
        ))?;
        let user_id = identity.user_id().ok_or(SecretonError::AwsError(
            "No user ID in identity".to_string(),
        ))?;
        let arn = identity
            .arn()
            .ok_or(SecretonError::AwsError("No ARN in identity".to_string()))?;
    ...
    async fn get_iam_user_info(&self, username: &str) -> AuthMethodResult<AwsUserInfo> {
        ...
        let user = iam_client
            ...
            .send()
            .await
            .map_err(|e| SecretonError::AwsError(format!("Failed to get IAM user: {}", e)))?;
    ...
    async fn get_ec2_instance_info(&self, instance_id: &str) -> AuthMethodResult<AwsInstanceInfo> {
        ...
        let instances = ec2_client
            ...
            .send()
            .await
            .map_err(|e| SecretonError::AwsError(format!("Failed to get EC2 instance: {}", e)))?;
        ...
        if let Some(reservation) = instances.reservations().first() {
            if let Some(instance) = reservation.instances().first() {
                ...
                Err(SecretonError::AwsError("Instance not found".to_string()))
            }
        } else {
            Err(SecretonError::AwsError("Instance not found".to_string()))
        }
