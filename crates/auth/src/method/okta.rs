...
impl OktaAuthMethod {
    ...
    async fn authenticate_with_okta(
        ...
        let config = self
            ...
            .ok_or_else(|| SecretonError::Configuration {
                message: "Okta config not set".to_string(),
            })?;
        ...
        let response = self
            ...
            .map_err(|err| SecretonError::Network {
                message: format!("Okta authentication request failed: {err}"),
            })?;
        ...
        let auth_response: OktaAuthResponse =
            response.json().await.map_err(|err| SecretonError::Parse {
                message: format!("Failed to parse Okta auth response: {err}"),
            })?;

        // Check if authentication was successful
        match auth_response.status.as_str() {
            "SUCCESS" => Ok(auth_response),
            "MFA_REQUIRED" | "MFA_CHALLENGE" => Err(SecretonError::MfaRequired),
            "LOCKED_OUT" => Err(SecretonError::AccountLocked {
                username: username.to_string(),
            }),
            "PASSWORD_EXPIRED" => Err(SecretonError::PasswordExpired {
                username: username.to_string(),
            }),
    ...
    async fn get_user_info(&self, id: &str) -> AuthMethodResult<OktaUser> {
        ...
        let response = self
            ...
            .await
            .map_err(|err| SecretonError::Network {
                message: format!("Okta user info request failed: {err}"),
            })?;

        if !response.status().is_success() {
            return Err(SecretonError::Authentication {
                message: format!("Failed to get Okta user info: {}", response.status()),
            });
        }

        response.json().await.map_err(|err| SecretonError::Parse {
            message: format!("Failed to parse Okta user response: {err}"),
        })
    }
    ...
    async fn get_user_groups(&self, id: &str) -> AuthMethodResult<Vec<OktaGroup>> {
        ...
        let response = self
            ...
            .await
            .map_err(|err| SecretonError::Network {
                message: format!("Okta groups request failed: {err}"),
            })?;

        if !response.status().is_success() {
            return Err(SecretonError::Authentication {
                message: format!("Failed to get Okta user groups: {}", response.status()),
            });
        }

        response.json().await.map_err(|err| SecretonError::Parse {
            message: format!("Failed to parse Okta groups response: {err}"),
        })
    }
...
impl AuthMethodImpl for OktaAuthMethod {
    ...
    async fn init(&mut self, config: &AuthMethod) -> AuthMethodResult<()> {
        ...
        if let (Some(org_url), Some(api_token)) = (
            config.config.get("org_url").and_then(|v| v.as_str()),
            ...
            AuthCredentials::UserPass { username, password } => {
                ...
                if !self.validate_group_membership(&user_groups, &config.allowed_groups) {
                    return Err(SecretonError::Authorization {
                        message: "Okta user is not in an allowed group".to_string(),
                    });
                }
                ...
                let user_info_result = UserInfo {
                    ...
                    username: user_info.profile.login.clone(),
                    ...
                    metadata: {
                        ...
                        if let Some(email) = &user_info.profile.email {
                            meta.insert("email".to_string(), email.clone());
                        }
                        if let Some(first_name) = &user_info.profile.first_name {
                            meta.insert("first_name".to_string(), first_name.clone());
                        }
                        if let Some(last_name) = &user_info.profile.last_name {
