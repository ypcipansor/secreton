...
impl Entity {
    ...
    /// Add metadata
    pub fn add_metadata(&mut self, _key: String, value: String) {
        self.metadata.insert(_key, value);
...
impl Alias {
    /// Create new alias
    pub fn new(entity_id: String, mount_accessor: String, _name: String) -> Self {
        Self {
            id: Uuid::new_v4().to_string(),
            entity_id,
            mount_accessor,
...
pub struct IdentityService {
    ...
    alias_index: Arc<RwLock<HashMap<String, String>>>, // (mount_accessor, _name) -> alias_id
}
...
impl IdentityService {
    ...
    /// Create entity
    pub async fn create_entity(&self, _name: String) -> Entity {
        let entity = Entity::new(_name);
    ...
    /// Get entity
    pub async fn get_entity(&self, id: &str) -> Result<Entity, SecretonError> {
    ...
    /// Update entity
    pub async fn update_entity(&self, id: &str, entity: Entity) -> Result<(), SecretonError> {
        let mut entities = self.entities.write().await;
    ...
    /// Delete entity
    pub async fn delete_entity(&self, id: &str) -> Result<(), SecretonError> {
        let mut entities = self.entities.write().await;
        ...
        for alias_id in alias_ids {
            if let Some(alias) = aliases.remove(&alias_id) {
                let _key = format!("{}:{}", alias.mount_accessor, alias._name);
                alias_index.remove(&_key);
            }
        }
    ...
    pub async fn create_alias(
        ...
        _name: String,
    ) -> Result<Alias, SecretonError> {
        let entities = self.entities.read().await;

        if !entities.contains_key(&entity_id) {
            return Err(SecretonError::EntityNotFound { entity: entity_id });
        }
        drop(entities);

        let _key = format!("{}:{}", mount_accessor, _name);
        let mut alias_index = self.alias_index.write().await;
        ...
        alias_index.insert(_key, alias.id.clone());
        drop(alias_index);
    ...
    /// Get alias
    pub async fn get_alias(&self, id: &str) -> Result<Alias, SecretonError> {
    ...
    /// Create group
    pub async fn create_group(&self, _name: String) -> Group {
        let group = Group::new(_name);
    ...
    /// Get group
    pub async fn get_group(&self, id: &str) -> Result<Group, SecretonError> {
