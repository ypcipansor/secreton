...
impl CertAuth {
    ...
    pub fn parse_certificate(&self, pem_data: &str) -> Result<CertificateSubject, SecretonError> {
        ...
        let der_data = base64.decode(&pem_body)
            .map_err(|_e| AuthError::certificate_parse_error(format!("Base64 decode error: {}", _e)))?;
        
        let (_, cert) = X509Certificate::from_der(&der_data)
            .map_err(|_e| AuthError::certificate_parse_error(format!("DER parse error: {}", _e)))?;
    ...
    pub async fn validate_certificate(&self, cert: &CertificateSubject) -> Result<(), SecretonError> {
        ...
        if now < cert.not_before {
            return Err(AuthError::certificate_not_yet_valid());
        }
        if now > cert.not_after {
            return Err(AuthError::certificate_expired());
        }
        
        // Check revocation
        if revoked.contains(&cert.serial_number) {
            return Err(AuthError::certificate_revoked());
        }
        
        // Validate against trusted CAs (simplified - in production would verify chain)
        if _config.trusted_cas.is_empty() {
            return Err(AuthError::certificate_untrusted_ca());
        }
    ...
    pub async fn authenticate(&self, pem_data: &str) -> Result<UserInfo, SecretonError> {
        ...
        Ok(UserInfo {
            _username: cert.common_name.clone(),
