...
pub struct Token {
    ...
    /// Entity ID (_user/role that owns this token)
...
impl Token {
    ...
    pub fn use_token(&mut self) -> Result<(), SecretonError> {
        if self.num_uses > 0 {
            self.num_uses -= 1;
            if self.num_uses == 0 {
                self.revoke();
...
impl TokenService {
    ...
    pub async fn verify_token(&self, token_value: &str) -> Result<Token, SecretonError> {
    ...
    /// Revoke token
    pub async fn revoke_token(&self, token_value: &str) -> Result<(), SecretonError> {
        let token_by_value = self.token_by_value.read().await;
        let token_id = token_by_value
            .get(token_value)
            .ok_or_else(|| SecretonError::TokenNotFound {
