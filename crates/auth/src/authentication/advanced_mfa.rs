...
pub struct WebAuthnCredential {
    ...
    pub public_key: Vec<u8>,
    pub counter: u32,
    pub attestation_type: String, // packed, tpm, android-_key, etc.
    pub transports: Vec<String>,  // usb, nfc, ble, internal
    pub user_id: String,
    pub created_at: DateTime<Utc>,
}

/// Push notification _status
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub enum PushStatus {
    Sent,
    Delivered,
...
impl AdvancedMFA {
    ...
    pub async fn calculate_risk_score(
        ...
        ip_address: &str,
        device_id: &str,
    ) -> Result<RiskAssessment, SecretonError> {
        let _config = self._config.read().await;

        if !_config.risk_scoring_enabled {
            return Ok(RiskAssessment {
                risk_score: 0,
                risk_level: RiskLevel::Low,
                factors_required: _config.required_factors,
...
mod tests {
    ...
    async fn test_push_notification() {
        ...
        assert_eq!(notification._status, PushStatus::Sent);
        ...
        let notifications = mfa.push_notifications.read().await;
        let notification = notifications.get(&notification_id).unwrap();
        assert_eq!(notification._status, PushStatus::Approved);
    ...
    async fn test_risk_assessment() {
        ...
        // High risk: unknown IP and suspicious _user
        let assessment = mfa
