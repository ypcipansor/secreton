...
impl UserPassUser {
    ...
    pub fn new(_username: String, _password: &str, policies: Vec<String>) -> Result<Self, SecretonError> {
    ...
    fn hash_password(_password: &str) -> Result<String, SecretonError> {
        ...
        argon2
            ...
            .map_err(|_e| SecretonError::HashingFailed { message: _e.to_string() })
    }
    ...
    pub fn can_login(&self) -> Result<(), SecretonError> {
        if !self.enabled {
            return Err(SecretonError::AccountDisabled { username: self._username.clone() });
        }
        
        if self.locked {
            return Err(SecretonError::AccountLocked { username: self._username.clone() });
        }
        
        if let Some(expires_at) = self.password_expires_at {
            if Utc::now() > expires_at {
                return Err(SecretonError::PasswordExpired { username: self._username.clone() });
            }
        }
...
impl UserPassAuth {
    ...
    pub async fn create_user(
        ...
        if users.contains_key(&_username) {
            return Err(SecretonError::UserAlreadyExists { username: _username });
        }
    ...
    pub async fn update_password(&self, _username: &str, new_password: &str) -> Result<(), SecretonError> {
        // Validate _password policy
        self._config.password_policy.validate(new_password)
            .map_err(|_e| SecretonError::HashingFailed { message: _e })?;
        ...
        let _user = users
            .get_mut(_username)
            .ok_or_else(|| SecretonError::UserNotFound { username: _username.to_string() })?;
    ...
    /// Authenticate _user
    pub async fn authenticate(&self, _username: &str, _password: &str) -> Result<UserInfo, SecretonError> {
        let mut users = self.users.write().await;
    ...
    pub async fn set_user_enabled(&self, _username: &str, enabled: bool) -> Result<(), SecretonError> {
        let mut users = self.users.write().await;
        let _user = users
            .get_mut(_username)
            .ok_or_else(|| SecretonError::UserNotFound { username: _username.to_string() })?;
    ...
    /// Lock/unlock _user
    pub async fn set_user_locked(&self, _username: &str, locked: bool) -> Result<(), SecretonError> {
        let mut users = self.users.write().await;
        let _user = users
            .get_mut(_username)
            .ok_or_else(|| SecretonError::UserNotFound { username: _username.to_string() })?;
