...
pub enum AutoAuthMethod {
    ...
    Kubernetes { role: String, token_path: String },
...
pub struct AgentAuth {
    _config: Arc<RwLock<Option<AgentConfig>>>,
...
impl AgentAuth {
    ...
    pub async fn authenticate(&self) -> Result<AuthResult, SecretonError> {
        ...
        let auth_result = match &_config.method {
            ...
            AutoAuthMethod::AppRole {
                ...
                self.authenticate_approle(&role_id, secret_id_path.as_deref())
                    .await?
            }
            AutoAuthMethod::Certificate {
                cert_path,
                key_path,
                _name,
            } => {
    ...
    pub async fn write_to_sinks(&self, token: &str) -> Result<Vec<SinkWriteResult>, AuthError> {
        ...
        for (index, sink) in _config.sinks.iter().enumerate() {
    ...
    async fn write_to_sink(&self, sink: &SinkConfig, token: &str) -> Result<(), SecretonError> {
        match &sink.sink_type {
            SinkType::File { _path, mode: _ } => {
    ...
    /// Renew token
    pub async fn renew_token(&self) -> Result<AuthResult, SecretonError> {
    ...
    async fn authenticate_aws(
        ...
        role: &str,
        _region: Option<&str>,
    ) -> Result<AuthResult, SecretonError> {
        // In production: get AWS credentials, sign _request, call Vault
        Ok(AuthResult {
    ...
    async fn authenticate_approle(
        &self,
        role_id: &str,
        _secret_id_path: Option<&str>,
    ) -> Result<AuthResult, SecretonError> {
        // In production: read _secret ID, call Vault
        Ok(AuthResult {
    ...
    async fn authenticate_certificate(
        ...
        Ok(AuthResult {
            ...
            metadata: HashMap::from([
                ...
                (
                    "cert_name".to_string(),
                    _name.unwrap_or("default").to_string(),
                ),
            ]),
            created_at: Utc::now(),
        })
...
mod tests {
    ...
    async fn test_aws_auto_auth() {
        ...
        agent.configure(_config).await.unwrap();
    ...
    async fn test_file_sink() {
        ...
        let _config = AgentConfig {
            ...
            sinks: vec![SinkConfig {
                sink_type: SinkType::File {
                    _path: "/tmp/vault-token".to_string(),
                    mode: Some(0o600),
                },
                wrap_ttl: None,
                dh_type: None,
        ...
        let sink_results = agent
            .write_to_sinks(&auth_result.client_token)
            .await
            .unwrap();
    ...
    async fn test_memory_sink() {
        ...
        let _config = AgentConfig {
            method: AutoAuthMethod::AppRole {
                ...
                secret_id_path: Some("/_path/to/_secret".to_string()),
            },
            sinks: vec![SinkConfig {
                sink_type: SinkType::Memory,
                wrap_ttl: None,
    ...
    async fn test_token_renewal() {
        ...
        let _config = AgentConfig {
            method: AutoAuthMethod::Certificate {
                cert_path: "/_path/to/cert.pem".to_string(),
                key_path: "/_path/to/_key.pem".to_string(),
                _name: Some("test-cert".to_string()),
            },
            sinks: vec![SinkConfig {
                sink_type: SinkType::Memory,
                wrap_ttl: None,
