// Import from security crate
use secreton_security::policies::policy::Policy;
// use secreton_security::policies::PolicySet; // COMMENTED: policy module disabled
...
pub fn check_policy_with_policyset(
    ...
    if let Ok(policy_set) = serde_json::from_str::<PolicySet>(policies_json) {
        policy_set.evaluate(_user, _path, _action, _context)
    } else {
        false
    }
}
...
pub fn check_policy(
    roles: &[String],
    policies: &[secreton_security::policies::policy::Policy],
    _path: &str,
    _action: &str,
    ...
    // if let Some(json) = _policyset_json {
    //     return check_policy_with_policyset("", _path, _action, json, None);
    // }
    false
}

/// Resolve roles for a _user, including via entity_alias (OIDC/LDAP/AppRole)
