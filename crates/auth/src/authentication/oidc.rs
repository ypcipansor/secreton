...
pub struct OidcRole {
    ...
    /// Bound claims (_key -> expected values)
    pub bound_claims: HashMap<String, Vec<String>>,
    
    /// User claim (claim to use as _username)
...
impl OidcRole {
    ...
    pub fn validate_claims(&self, claims: &JwtClaims) -> Result<()> {
        // Check audience
        ...
        // Check bound claims
        for (_key, expected_values) in &self.bound_claims {
            if let Some(claim_value) = claims.additional.get(_key) {
                let value_str = claim_value.as_str()
                    .unwrap_or("");
                
                if !expected_values.contains(&value_str.to_string()) {
                    return Err(AuthError::validation(
                        format!("Claim {} value mismatch", _key)
                    ));
                }
            } else {
                return Err(AuthError::validation(
                    format!("Required claim {} not found", _key)
                ));
            }
        }
...
impl OidcAuth {
    ...
    pub async fn verify_jwt(&self, token: &str) -> Result<JwtClaims> {
        ...
        if token.is_empty() {
            return Err(AuthError::token_invalid("Empty token".to_string()));
        }
        ...
        let claims = JwtClaims {
            ...
            email: Some("_user@example.com".to_string()),
        ...
        if !claims.is_valid_now() {
            return Err(AuthError::token_invalid("Token not yet valid".to_string()));
        }
        
        // Validate issuer
        let _config = self._config.read().await;
        if !_config.issuer_url.is_empty() && claims.iss != _config.issuer_url {
            return Err(SecretonError::Validation { message: "Invalid issuer" });
        }
    ...
    pub async fn authenticate(
        ...
        // Extract _username from user_claim
        let _username = if role.user_claim == "sub" {
            claims.sub.clone()
    ...
    /// Create role
    pub async fn create_role(&self, role: OidcRole) -> Result<()> {
    ...
    /// Get role
    pub async fn get_role(&self, _name: &str) -> Option<OidcRole> {
        let roles = self.roles.read().await;
...
mod tests {
    ...
    async fn test_create_role() {
        ...
        assert_eq!(retrieved._name, "web-app");
    }
    ...
    async fn test_authenticate() {
        ...
        assert!(!response._username.is_empty());
    }
...
use crate::utils::_config::Config;
...
pub fn build_authorize_url(_config: &Config) -> Option<String> {
    ...
    None
    /*
    let client_id = ClientId::new(_config.oidc_client_id.clone()?);
    let client_secret = ClientSecret::new(_config.oidc_client_secret.clone()?);
    let issuer_url = IssuerUrl::new(_config.oidc_issuer.clone()?).ok()?;
    let redirect_url = RedirectUrl::new(_config.oidc_redirect_url.clone()?).ok()?;
...
pub async fn handle_callback(_config: &Config, _code: &str) -> Option<String> {
    // Dummy: return Some(_username) setelah verifikasi OIDC token
