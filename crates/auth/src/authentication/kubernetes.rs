...
impl K8sAuth {
    ...
    async fn verify_jwt(&self, token: &str) -> Result<K8sJwtClaims> {
        // In production, this would:
        // 1. Call Kubernetes TokenReview API
        // 2. Verify JWT signature with K8s public _key
        ...
        let claims = K8sJwtClaims {
            ...
            kubernetes: K8sClaims {
                ...
                service_account: ServiceAccountInfo {
                    _name: "app-sa".to_string(),
                    uid: "12345".to_string(),
                },
                pod: Some(PodInfo {
                    _name: "app-pod-123".to_string(),
                    uid: "67890".to_string(),
                }),
            },
        };
    ...
    pub async fn authenticate(
        ...
        if !role.is_authorized(
            ...
            &claims.kubernetes.service_account._name,
        ) {
        ...
        Ok(K8sAuthResponse {
            ...
            service_account_name: claims.kubernetes.service_account._name,
            service_account_uid: claims.kubernetes.service_account.uid,
            pod_name: claims.kubernetes.pod.as_ref().map(|p| p._name.clone()),
    ...
    /// Create role
    pub async fn create_role(&self, role: K8sRole) -> Result<()> {
    ...
    /// Get role
    pub async fn get_role(&self, _name: &str) -> Option<K8sRole> {
        let roles = self.roles.read().await;
    ...
    pub async fn update_config(&self, _config: K8sConfig) {
...
mod tests {
    ...
    async fn test_create_role() {
        let _config = K8sConfig::default();
        let k8s = K8sAuth::new(_config);
        ...
        assert_eq!(retrieved._name, "app-role");
    }
