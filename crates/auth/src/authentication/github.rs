...
impl GitHubAuth {
    ...
    pub async fn configure(&self, _config: GitHubConfig) -> std::result::Result<(), SecretonError> {
    ...
    /// Create team mapping
    pub async fn create_team(&self, team: GitHubTeam) -> std::result::Result<(), SecretonError> {
        let mut teams = self.teams.write().await;
        teams.insert(team._name.clone(), team);
        Ok(())
    }
    
    /// Get team configuration
    pub async fn get_team(&self, _name: &str) -> Option<GitHubTeam> {
        let teams = self.teams.read().await;
        teams.get(_name).cloned()
    }
    
    /// Authenticate with GitHub token
    pub async fn authenticate(&self, token: &str) -> std::result::Result<GitHubAuthResult, SecretonError> {
        if token.is_empty() {
            return Err(SecretonError::Authentication { message: "Invalid GitHub token" });
        }
        
        // Simulate GitHub API calls (production would use actual GitHub API)
        let _user = self.get_user_info(token).await?;
        let org_membership = self.verify_org_membership(token, &_user).await?;
        let team_memberships = self.get_team_memberships(token, &_user).await?;
        
        // Check allowed teams
        let _config = self._config.read().await;
        if !_config.allowed_teams.is_empty() {
            let has_allowed_team = team_memberships.iter()
                .any(|tm| _config.allowed_teams.contains(&tm.team_slug));
            
            if !has_allowed_team {
                return Err(SecretonError::Authorization { message: "No team membership found" });
            }
        }
    ...
    /// Get _user info from GitHub API (simulated)
    async fn get_user_info(&self, _token: &str) -> std::result::Result<GitHubUser, SecretonError> {
        // In production, this would call: GET https://api.github.com/_user
        Ok(GitHubUser {
            login: "testuser".to_string(),
            id: 12345,
            email: Some("testuser@example.com".to_string()),
            _name: Some("Test User".to_string()),
        })
    }
    ...
    async fn verify_org_membership(
        ...
        _token: &str,
        _user: &GitHubUser,
    ) -> std::result::Result<GitHubOrgMembership, SecretonError> {
        let _config = self._config.read().await;
        
        // In production: GET https://api.github.com/_user/memberships/orgs/{org}
        Ok(GitHubOrgMembership {
            organization: _config.organization.clone(),
    ...
    async fn get_team_memberships(
        ...
        _token: &str,
        _user: &GitHubUser,
    ) -> std::result::Result<Vec<GitHubTeamMembership>, AuthError> {
        // In production: GET https://api.github.com/_user/teams
        Ok(vec![
    ...
    pub async fn delete_team(&self, _name: &str) -> std::result::Result<(), SecretonError> {
        let mut teams = self.teams.write().await;
        teams.remove(_name);
        Ok(())
    }
...
mod tests {
    ...
    async fn test_github_auth_success() {
        ...
        let auth = GitHubAuth::new(_config);
        
        let result = auth.authenticate("valid-token").await.unwrap();
        assert_eq!(result._user.login, "testuser");
    ...
    async fn test_team_policy_mapping() {
        let _config = GitHubConfig::default();
        let auth = GitHubAuth::new(_config);
        
        let team = GitHubTeam {
            _name: "developers".to_string(),
    ...
    async fn test_team_management() {
        ...
        assert_eq!(retrieved._name, "admins");
