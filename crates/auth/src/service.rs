...
pub type AuthMethodResult<T> = Result<T, SecretonError>;
...
impl AuthMethodService {
    ...
    pub async fn login(&self, request: &LoginRequest) -> AuthMethodResult<LoginResponse> {
        ...
        let auth_result = self.authenticate("userpass", &credentials).await?;

        let (success, token, user_info) = if auth_result.mfa_required && request.mfa_code.is_none()
        {
            (false, None, None)
        } else {
            (
