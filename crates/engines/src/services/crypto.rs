...
use secreton_crypto::{
    AlgorithmId, CryptoEngine, CryptoError, EncryptedData, KeyManager, KeyStorage, SecurityParams,
};
...
impl SystemKeyStorage {
    ...
    async fn encrypt_key(&self, key_data: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        let root_key = root_key_guard
            .as_ref()
            .ok_or(CryptoError::Internal("System is sealed".to_string()))?;

        let encrypted = self
            .crypto_engine
            .encrypt(AlgorithmId::Aes256Gcm, key_data, root_key)?;

        // Serialize EncryptedData to bytes
        serde_json::to_vec(&encrypted)
            .map_err(|e| CryptoError::Internal(format!("Serialization error: {}", e)))
    }
...
impl CryptoService {
    ...
    pub async fn new(storage: Arc<dyn StorageBackend + Send + Sync>) -> Result<Self> {
        ...
        let key_storage = Arc::new(SystemKeyStorage::new(
            storage.clone(),
        ...
        if root_key_store.read().await.is_some() && key_manager.get_active_key().await.is_err() {
    ...
    pub fn decrypt_full(
        ...
        Err(anyhow::anyhow!(
            "Direct low-level decryption via decrypt_full is deprecated. Use decrypt_with_key."
        ))
    }
    ...
    pub async fn encrypt_data(&self, plaintext: &[u8]) -> Result<Vec<u8>> {
        ...
        serde_json::to_vec(&packet).map_err(|e| anyhow::anyhow!("Serialization error: {}", e))
    }
...
mod tests {
    ...
    async fn test_crypto_service_lifecycle() {
        ...
        // NOTE: the service reads SECRETON_ROOT_KEY from the process environment.
        // `set_var` is `unsafe` in edition 2024 and races the other tests in this
        // binary, so the harness sets it once instead — see `set_test_root_key`.
    ...
    async fn test_signing_and_verification_ed25519() {
        ...
        let signature = service
            .sign_data(&key, data, AlgorithmId::Ed25519)
            .expect("Signing failed");

        assert_eq!(signature.len(), 64); // Ed25519 signatures are 64 bytes

        let verified = service
            .verify_signature(&key, data, &signature, AlgorithmId::Ed25519)
        ...
        let verified_invalid = service
            .verify_signature(&key, data, &invalid_signature, AlgorithmId::Ed25519)
            .expect("Verification failed");
    ...
    async fn test_signing_and_verification_p256() {
        ...
        let signature = service
            .sign_data(&key, data, AlgorithmId::EcdsaP256)
            .expect("Signing failed");

        let verified = service
            .verify_signature(&key, data, &signature, AlgorithmId::EcdsaP256)
            .expect("Verification failed");
