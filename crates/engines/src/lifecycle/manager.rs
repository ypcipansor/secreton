...
pub use secreton_domain::dto::lifecycle::{LifecycleStatistics, SecretLifecycle, SecretStatus};
...
impl SecretLifecycleManagement {
    ...
    /// Fire every enabled hook of `hook_type` for `secret_path`.
    ///
    /// A webhook failure is logged and the remaining hooks still run. It must not abort
    /// the caller: this is invoked from the expiry and archival paths, and propagating
    /// the error there meant one unreachable third-party endpoint left an expired secret
    /// live and skipped every hook registered after it.
    pub async fn trigger_lifecycle_hook(&self, hook_type: HookType, secret_path: &str) {
        ...
        for hook in relevant_hooks {
            if let Err(e) = self.call_webhook(&hook.action_url, secret_path).await {
                tracing::warn!(
...
mod tests {
    ...
    async fn test_expire_secret() {
        ...
        let hook = LifecycleHook {
            ...
            // Deliberately unroutable (RFC 5737 TEST-NET-1, discard port). The point of
            // the assertion below is that expiry completes even when the hook cannot be
            // delivered; a reachable URL would make this test depend on the network.
            action_url: "http://192.0.2.1:9/".to_string(),
            enabled: true,
        };
