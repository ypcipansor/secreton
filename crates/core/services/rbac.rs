...
pub fn check_policy_with_policyset(user: &str, path: &str, action: &str, policies_json: &str, context: Option<&Value>) -> bool {
...
pub fn check_policy(roles: &[String], policies: &[crate::models::policy::Policy], path: &str, action: &str, policyset_json: Option<&str>) -> bool {
    // Cek policy RBAC tradisional
    for policy in policies {
        if roles.contains(&policy.role) && policy.action == action && glob::Pattern::new(&policy.path).map(|p| p.matches(path)).unwrap_or(false) {
...
pub fn resolve_user_roles(user_id: &str, entity_alias: Option<&str>, policies: &[Policy]) -> Vec<String> {
...
mod tests {
    ...
    fn test_policy_globbing() {
        ...
        assert!(check_policy(&roles, &policies, "secret/data/foo", "read", None));
    ...
    fn test_entity_alias() {
        ...
        assert!(check_policy(&roles, &policies, "secret/data/bar", "read", None));
    }
} 