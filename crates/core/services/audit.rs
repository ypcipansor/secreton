...
impl AuditDevice for FileAuditDevice {
    fn log(&self, user: &str, action: &str, path: &str, status: &str) {
        let mut f = OpenOptions::new().create(true).append(true).open(&self.file).unwrap();
...
pub fn log_audit(devices: &[Box<dyn AuditDevice>], user: &str, action: &str, path: &str, status: &str) {
    for dev in devices {
...
pub fn log_audit_db(user: &str, action: &str, path: &str, status: &str) {
    // ... existing DB logic ...
}
...
pub async fn log_audit_external(devices: &[Box<dyn ExternalAuditDevice>], event: &str, user: &str, resource: &str, status: &str) {
