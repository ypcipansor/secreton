//! Metrics and monitoring for Brankas
...
impl MetricsCollector for InMemoryMetricsCollector {
    ...
    fn get_security_metrics(&self) -> SecurityMetrics {
        SecurityMetrics {
            ...
            password_violations: self.get_counter("security.password_violations").unwrap_or(0) as u32,
            suspicious_activities: self.get_counter("security.suspicious_activities").unwrap_or(0) as u32,
    ...
    fn get_performance_metrics(&self) -> PerformanceMetrics {
        PerformanceMetrics {
            db_avg_query_time: self.get_gauge("performance.db_avg_query_time").unwrap_or(0.0),
            ...
            encryption_ops_per_sec: self.get_gauge("performance.encryption_ops_per_sec").unwrap_or(0.0),
            decryption_ops_per_sec: self.get_gauge("performance.decryption_ops_per_sec").unwrap_or(0.0),
            avg_encryption_time: self.get_gauge("performance.avg_encryption_time").unwrap_or(0.0),
            avg_decryption_time: self.get_gauge("performance.avg_decryption_time").unwrap_or(0.0),
            timestamp: Utc::now(),
        }
    }
}
...
pub trait HealthCheckProvider: Send + Sync {
    fn name(&self) -> &str;
    fn check(&self) -> std::pin::Pin<Box<dyn std::future::Future<Output = (HealthStatus, Option<String>)> + Send + '_>>;
}
