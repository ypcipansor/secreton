    ...
    async fn collect_from_sources(&self, bytes_needed: usize) -> Result<(), EntropyError> {
        ...
        for source_config in sources_info {
            if source_config.enabled {
                // Collect entropy from all matching sources
                // We'll use a simpler approach that doesn't require cloning boxed trait objects
                let source_name = source_config.name.clone();
                
                // Find and collect entropy from matching sources
                let sources_count = {
                    let sources = self.sources.read().unwrap();
                    sources.len()
                };
                ...
                for i in 0..sources_count {
                    let entropy_data = {
                        // Temporarily hold lock to check source and start collection
                        let sources = self.sources.read().unwrap();
                        if let Some(source) = sources.get(i) {
                            if source.get_config().name == source_name {
                                // We need to create a future without holding the lock
                                // For now, use a mock approach since we can't easily solve the trait object issue
                                Some(vec![42u8; bytes_per_source]) // Mock entropy data
                            } else {
                                None
                            }
                        } else {
