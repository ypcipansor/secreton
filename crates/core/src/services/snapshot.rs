...
impl SnapshotService {
    ...
    pub async fn restore_snapshot(
        ...
        let snapshot_data = self.snapshot_data.read().await;
        let data = snapshot_data.get(snapshot_id)
            .ok_or_else(|| SnapshotError::NotFound(snapshot_id.to_string()))?
            .clone();
        drop(snapshot_data);
...
mod tests {
    ...
    async fn test_create_full_snapshot() {
        ...
        service.update_state(|state| {
            ...
            state.policies.insert("policy1".to_string(), "rule1".to_string());
        }).await;
    ...
    async fn test_restore_snapshot() {
        ...
        // Restore
        let result = service.restore_snapshot(
            &snapshot.id,
            RestoreOptions {
                force: true,
                ..Default::default()
    ...
    async fn test_incremental_snapshot() {
        ...
        // Create incremental snapshot
        let incremental = service.create_incremental_snapshot(
    ...
    async fn test_list_and_delete_snapshots() {
        ...
        let snapshot1 = service.create_snapshot(Some("First".to_string()), false).await.unwrap();
        let snapshot2 = service.create_snapshot(Some("Second".to_string()), false).await.unwrap();
