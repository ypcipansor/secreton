...
impl AzureKeyVaultBackend {
    ...
    async fn mock_vault_secret_exists(&self, vault_path: &str) -> bool {
        true
    }

    async fn mock_update_vault(&self, vault_path: &str, value: &str) -> Result<()> {
        Ok(())
    }

    async fn mock_get_vault_value(&self, vault_path: &str) -> Result<String> {
        Ok("vault-value".to_string())
    }

    async fn mock_create_vault(&self, vault_path: &str, value: &str) -> Result<()> {
        Ok(())
    }
