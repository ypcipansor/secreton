...
pub struct HESystem {
    ...
    secret_keys: Arc<RwLock<HashMap<String, SecretKey>>>,
...
impl HESystem {
    ...
    pub async fn generate_keypair(&self, scheme: HEScheme) -> Result<(String, String)> {
        ...
        let mut pub_keys = self.public_keys.write().await;
        let mut sec_keys = self.secret_keys.write().await;

        pub_keys.insert(key_id.clone(), public_key);
        sec_keys.insert(key_id.clone(), secret_key);
    ...
    pub async fn encrypt(&self, public_key_id: &str, plaintext: &[u8], owner: &str) -> Result<Ciphertext> {
        ...
        // Mock encryption (real implementation would use proper HE schemes)
    ...
    pub async fn decrypt(&self, secret_key_id: &str, ciphertext: &Ciphertext) -> Result<Vec<u8>> {
        let sec_keys = self.secret_keys.read().await;
