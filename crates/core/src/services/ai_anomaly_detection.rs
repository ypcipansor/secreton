...
impl AnomalyDetectionSystem {
    ...
    async fn create_anomaly(&self, event: &AccessEvent) -> Result<()> {
        let anomaly = Anomaly {
    ...
    async fn generate_remediation(&self, anomaly: &Anomaly) -> Result<()> {
        ...
        remediations.insert(action.action_id.clone(), action);
    ...
    pub async fn establish_baseline(&self, user_id: &str) -> Result<()> {
        ...
        patterns.insert(user_id.to_string(), pattern);
    ...
    pub async fn apply_remediation(&self, action_id: &str) -> Result<()> {
        ...
        println!("Applying remediation: {:?}", action.action_type);
    ...
    pub async fn record_threat(&self, threat: ThreatIntelligence) -> Result<String> {
    ...
    /// Get model
    pub async fn get_model(&self, model_id: &str) -> Result<AnomalyModel> {
        let models = self.models.read().await;
        models
            .get(model_id)
            .cloned()
            .ok_or_else(|| AnomalyError::ModelNotFound(model_id.to_string()))
    }
...
mod tests {
    ...
    async fn test_establish_baseline() {
        ...
        assert!(patterns.contains_key("bob"));
    }
