...
impl NamespaceTree {
    ...
    pub fn get_by_path(&self, path: &str) -> Option<&Namespace> {
        self.namespaces.values().find(|ns| ns.path == path)
    }
...
impl NamespaceService {
    ...
    pub async fn list(&self, parent_id: Option<&str>) -> Vec<Namespace> {
        ...
        if let Some(parent) = parent_id {
            tree.get_children(parent).into_iter().cloned().collect()
        } else {
            tree.list_all().into_iter().cloned().collect()
        }
    }
    ...
    pub async fn resolve_path(
        &self,
        full_path: &str,
    ) -> Result<(Namespace, String), NamespaceError> {
    ...
    pub async fn update(
        &self,
        id: &str,
        update_fn: impl FnOnce(&mut Namespace),
