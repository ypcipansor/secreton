...
//!
//! Username/password authentication for Secreton.
...
impl UserPassUser {
    ...
    pub fn update_password(&mut self, new_password: &str) -> Result<(), UserPassError> {
    ...
    pub fn can_login(&self) -> Result<(), UserPassError> {
        if !self.enabled {
            return Err(UserPassError::AccountDisabled);
        }
...
impl UserPassAuth {
    ...
    pub async fn create_user(
        ...
        if users.contains_key(&username) {
            return Err(UserPassError::UserAlreadyExists(username));
        }
        
        let mut user = UserPassUser::new(username.clone(), password, policies)?;
        user.metadata = metadata;
        
        // Set password expiration if configured
        if let Some(days) = self.config.password_policy.expiration_days {
            user.password_expires_at = Some(Utc::now() + Duration::days(days as i64));
        }
    ...
    pub async fn update_password(&self, username: &str, new_password: &str) -> Result<(), UserPassError> {
        ...
        let user = users
            .get_mut(username)
            .ok_or_else(|| UserPassError::UserNotFound(username.to_string()))?;
    ...
    pub async fn authenticate(&self, username: &str, password: &str) -> Result<UserInfo, UserPassError> {
        ...
        if !user.verify_password(password)? {
            user.record_failure(self.config.max_failed_attempts);
        ...
        Ok(UserInfo {
            ...
            email: user.metadata.get("email").cloned(),
            display_name: user.metadata.get("display_name").cloned(),
    ...
    /// Lock/unlock user
    pub async fn set_user_locked(&self, username: &str, locked: bool) -> Result<(), UserPassError> {
        let mut users = self.users.write().await;
        let user = users
            .get_mut(username)
            .ok_or_else(|| UserPassError::UserNotFound(username.to_string()))?;
...
mod tests {
    ...
    async fn test_account_lockout() {
        ...
        assert!(matches!(result, Err(UserPassError::AccountLocked)));
    }
}
