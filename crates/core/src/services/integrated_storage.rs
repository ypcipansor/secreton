...
pub struct RaftConfig {
    ...
    pub snapshot_threshold: u64, // Log entries before snapshot
...
impl IntegratedRaftStorage {
    ...
    pub async fn add_node(&self, node: RaftNode) -> Result<()> {
    ...
    pub async fn start_election(&self) -> Result<bool> {
        ...
        for node in nodes.values() {
            let response = self.handle_vote_request_internal(&vote_request, node).await;
    ...
    /// Write data (leader forwards to Raft log)
    ...
    pub async fn delete(&self, key: String) -> Result<()> {
        let index = self
            .append_log_entry(LogOperation::Delete { key: key.clone() })
            .await?;
