...
impl LeaseManager {
    ...
    pub async fn revoke_lease(&self, lease_id: &str) -> Result<Vec<String>, LeaseError> {
        ...
        for child_id in child_ids {
            if let Ok(mut child_revoked) = self.revoke_lease(&child_id).await {
    ...
    /// List leases by user
    pub async fn list_by_user(&self, user: &str) -> Vec<EnhancedLease> {
        let by_user = self.by_user.read().await;
        let lease_ids = by_user.get(user).cloned().unwrap_or_default();
        drop(by_user);
