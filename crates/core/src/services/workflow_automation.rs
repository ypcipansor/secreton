...
impl WorkflowAutomation {
    ...
    async fn execute_dag(
        ...
        loop {
            let node = dag.nodes.iter().find(|n| n.node_id == current_node_id);
            if let Some(node) = node {
                // Execute node action
                self.execute_action(&node.action, execution).await?;
                execution.completed_nodes.push(node.node_id.clone());

                // Find next node
                let next_edge = dag.edges.iter().find(|e| e.from_node == current_node_id);
    ...
    async fn execute_action(
        ...
        match action {
            ...
            WorkflowAction::RotateSecret(path) => {
                // Simulate rotation
                tokio::time::sleep(tokio::time::Duration::from_millis(10)).await;
            }
            WorkflowAction::NotifyTeam(channel) => {
            ...
            dag: WorkflowDAG {
                nodes: vec![
                    ...
                    WorkflowNode {
                        ...
                        action: WorkflowAction::NotifyTeam("team-security".to_string()),
                        conditions: vec![],
