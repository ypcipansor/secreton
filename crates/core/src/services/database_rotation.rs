...
impl DatabaseRootRotation {
    ...
    pub async fn rotate_root(&self, database_name: &str) -> Result<RootCredential, RootRotationError> {
        // Check if database exists
        let configs = self.configs.read().await;
        let config = configs.get(database_name)
        ...
        let old_credential = credentials.values()
            .find(|c| c.is_active)
            .cloned()
            .ok_or_else(|| RootRotationError::RotationFailed("No active credential found".to_string()))?;
        drop(credentials);
    ...
    pub async fn schedule_rotation(&self, database_name: &str) -> Result<DateTime<Utc>, RootRotationError> {
        let configs = self.configs.read().await;
        let config = configs.get(database_name)
            .ok_or_else(|| RootRotationError::DatabaseNotFound(database_name.to_string()))?;
    ...
    pub async fn get_next_rotation(&self, database_name: &str) -> Option<DateTime<Utc>> {
        let credentials = self.credentials.read().await;
    ...
    pub async fn get_status(&self, database_name: &str) -> RotationStatus {
        ...
        status.get(database_name)
            .cloned()
            .unwrap_or(RotationStatus::Idle)
    }
    
    /// Verify connection with credential (simulated)
    async fn verify_connection(&self, _credential: &RootCredential) -> Result<(), RootRotationError> {
    ...
    fn generate_password(&self) -> String {
        ...
        format!("password-{}", uuid::Uuid::new_v4())
    }
...
mod tests {
    ...
    async fn test_rotate_root() {
        ...
        service.configure(config, initial_credential.clone()).await.unwrap();
