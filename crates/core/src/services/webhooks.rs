...
pub struct WebhookSystem {
    config: Arc<RwLock<WebhookSystemConfig>>,
...
impl WebhookSystem {
    pub fn new(config: WebhookSystemConfig) -> Self {
        let webhooks = config
            .webhooks
            .iter()
    ...
    async fn deliver_to_webhook(
        ...
        let (status, response_code) = self.mock_http_post(webhook, &payload, &signature).await;
    ...
    fn generate_hmac_signature(&self, payload: &str, secret: &str) -> String {
        // Mock HMAC generation
        // Real implementation would use hmac crate with sha2
        format!("sha256={}", self.mock_hash(payload, secret))
    }

    fn mock_hash(&self, payload: &str, secret: &str) -> String {
        // Simple mock hash
        format!("{:x}", (payload.len() + secret.len()) * 123456789)
    }
    ...
    pub fn validate_webhook(&self, payload: &str, signature: &str, secret: &str) -> bool {
        let expected = self.generate_hmac_signature(payload, secret);
        expected == signature
    }
    ...
    pub async fn retry_delivery(&self) -> Result<usize> {
        ...
        let retry_delay = Duration::milliseconds(config.retry_delay_ms as i64);
        drop(config);
    ...
    /// Get delivery status for webhook
...
mod tests {
    ...
    async fn test_emit_event_to_webhook() {
        ...
        let system = WebhookSystem::new(config);
