...
impl ControlGroup {
    ...
    pub fn is_approved(&self) -> bool {
        let approved_count = self.authorizations.iter()
...
impl ControlGroupService {
    ...
    pub async fn authorize(
        ...
        let control_group = control_groups.get_mut(id)
    ...
    pub async fn check_approved(&self, id: &str) -> Result<bool, ControlGroupError> {
        ...
        let control_group = control_groups.get(id)
            .ok_or_else(|| ControlGroupError::NotFound(id.to_string()))?;
    ...
    pub async fn cleanup_expired(&self) -> usize {
        ...
        let expired_ids: Vec<String> = control_groups.values()
...
mod tests {
    ...
    async fn test_authorization_workflow() {
        ...
        // First authorization
        service.authorize(&id, "admin1".to_string(), true, Some("Approved".to_string())).await.unwrap();
        assert!(!service.check_approved(&id).await.unwrap());
        
        // Second authorization
        service.authorize(&id, "admin2".to_string(), true, None).await.unwrap();
        assert!(!service.check_approved(&id).await.unwrap());
        
        // Third authorization - should approve
        service.authorize(&id, "admin3".to_string(), true, None).await.unwrap();
        assert!(service.check_approved(&id).await.unwrap());
    ...
    async fn test_denial() {
        ...
        service.authorize(&id, "admin1".to_string(), true, None).await.unwrap();
        service.authorize(&id, "admin2".to_string(), false, Some("Rejected".to_string())).await.unwrap();
