...
mod tests {
    ...
    async fn test_write_read() {
        ...
        data.insert("api_key".to_string(), Value::String("secret123".to_string()));
        
        cubbyhole.write("token1", "config", data.clone()).await.unwrap();
        
        let read_data = cubbyhole.read("token1", "config").await.unwrap();
        assert_eq!(read_data.get("api_key"), Some(&Value::String("secret123".to_string())));
    }
    ...
    async fn test_isolation() {
        ...
        assert_eq!(token1_data.get("key"), Some(&Value::String("value1".to_string())));
        
        let token2_data = cubbyhole.read("token2", "data").await.unwrap();
        assert_eq!(token2_data.get("key"), Some(&Value::String("value2".to_string())));
    ...
    async fn test_cleanup_token() {
        ...
        cubbyhole.write("token1", "data1", data.clone()).await.unwrap();
        cubbyhole.write("token1", "data2", data.clone()).await.unwrap();
        cubbyhole.write("token1", "data3", data.clone()).await.unwrap();
    ...
    async fn test_list() {
        ...
        cubbyhole.write("token1", "app/config", data.clone()).await.unwrap();
        cubbyhole.write("token1", "app/credentials", data.clone()).await.unwrap();
