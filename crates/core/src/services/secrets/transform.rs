...
impl TransformEngine {
    ...
    pub async fn create_transformation(&self, transformation: Transformation) -> Result<(), TransformError> {
    ...
    pub async fn encode(
        ...
        if !role.transformations.contains(&transformation_name.to_string()) {
            return Err(TransformError::TransformationNotFound(transformation_name.to_string()));
        }
        drop(roles);
        
        // Get transformation
        let transformations = self.transformations.read().await;
        let transformation = transformations.get(transformation_name)
        ...
        match transformation.transformation_type {
            ...
            TransformationType::Tokenization => self.encode_tokenization(&transformation, value).await,
    ...
    async fn encode_fpe(&self, transformation: &Transformation, value: &str) -> Result<String, TransformError> {
        let alphabet = transformation.alphabet.as_ref()
        ...
        let encoded: String = value.chars()
            .map(|c| {
    ...
    /// Decode FPE
    async fn decode_fpe(&self, transformation: &Transformation, value: &str) -> Result<String, TransformError> {
        let alphabet = transformation.alphabet.as_ref()
            .ok_or(TransformError::InvalidAlphabet)?;
        ...
        let decoded: String = value.chars()
            .map(|c| {
                if let Some(pos) = chars.iter().position(|&ch| ch == c) {
    ...
    async fn encode_tokenization(&self, _transformation: &Transformation, value: &str) -> Result<String, TransformError> {
    ...
    async fn decode_tokenization(&self, token: &str) -> Result<String, TransformError> {
        ...
        let mapping = mappings.get(token)
    ...
    /// Masking encoding
    async fn encode_masking(&self, transformation: &Transformation, value: &str) -> Result<String, TransformError> {
    ...
    fn apply_template(&self, value: &str, template: &str, mask_char: char) -> Result<String, TransformError> {
        let value_chars: Vec<char> = value.chars().filter(|c| c.is_alphanumeric()).collect();
        let mut value_idx = 0;
        
        let result: String = template.chars()
            .map(|t| {
                match t {
                    '#' => {
...
mod tests {
    ...
    async fn test_tokenization() {
        ...
        let role = TransformRole::new(
            "payment".to_string(),
            vec!["cc-token".to_string()],
        );
        engine.create_role(role).await.unwrap();
        
        let card_number = "4111111111111111";
        let token = engine.encode("payment", "cc-token", card_number).await.unwrap();
        ...
        let token2 = engine.encode("payment", "cc-token", card_number).await.unwrap();
    ...
    async fn test_masking() {
        ...
        let role = TransformRole::new(
            "display".to_string(),
            vec!["cc-mask".to_string()],
        );
        engine.create_role(role).await.unwrap();
        
        let masked = engine.encode("display", "cc-mask", "4111222233334444").await.unwrap();
