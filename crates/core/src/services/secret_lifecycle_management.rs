...
pub struct SecretLifecycleManagement {
    config: Arc<RwLock<LifecycleConfig>>,
...
mod tests {
    ...
    async fn test_set_expiration() {
        ...
        let lifecycle = lifecycle_mgmt
            .set_expiration("secret/db/password".to_string(), 90)
            .await
            .unwrap();
    ...
    async fn test_check_expiring_secrets() {
        ...
        lifecycle_mgmt
            .set_expiration("secret/expiring".to_string(), 5)
            .await
            .unwrap();

        lifecycle_mgmt
            .set_expiration("secret/active".to_string(), 90)
            .await
            .unwrap();

        let expiring = lifecycle_mgmt.check_expiring_secrets(7).await;
        assert_eq!(expiring.len(), 1);
        assert_eq!(expiring[0].secret_path, "secret/expiring");
    }
    ...
    async fn test_expire_secret() {
        ...
        lifecycle_mgmt
            .set_expiration("secret/test".to_string(), 90)
            .await
            .unwrap();
        ...
        lifecycle_mgmt.expire_secret("secret/test").await.unwrap();

        let lifecycle = lifecycle_mgmt.get_lifecycle("secret/test").await.unwrap();
    ...
    async fn test_archive_secret() {
        ...
        lifecycle_mgmt
            .set_expiration("secret/archive".to_string(), 90)
            .await
            .unwrap();
    ...
    async fn test_extend_ttl() {
        ...
        lifecycle_mgmt
            .set_expiration("secret/extend".to_string(), 30)
            .await
            .unwrap();
