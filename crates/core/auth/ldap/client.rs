...
use super::config::{LdapConfig, LdapConnectionConfig, LdapUser, LdapGroup};
...
impl LdapClient {
    ...
    fn get_user_info(&self, conn: &mut LdapConn, username: &str, user_dn: &str) -> Result<LdapUser> {
        ...
        if rs.0.is_empty() {
            return Err(anyhow::anyhow!("User {} not found in attribute search", username));
        }
    ...
    fn get_user_groups(&self, conn: &mut LdapConn, user_dn: &str) -> Result<Vec<String>> {
        let filter = if let Some(group_filter) = &self.config.group_filter {
            format!("(&{}({}={}))", group_filter, self.config.group_attr, user_dn)
        } else {
    ...
    pub fn get_group_info(&self, group_name: &str) -> Result<LdapGroup> {
        ...
        let entry = SearchEntry::construct(rs.0[0].clone());
        let members = entry.attrs
            .get(&self.config.group_attr)
            .map(|v| v.clone())
            .unwrap_or_default();
        
        let policies = self.config.group_policy_mappings
            .get(group_name)
            .map(|v| v.clone())
            .unwrap_or_default();
