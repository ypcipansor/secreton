...
pub use config::{LdapAuthRequest, LdapAuthResponse, LdapConfig, LdapGroup, LdapUser};
...
impl AuthMethod for LdapAuth {
    ...
    async fn authenticate(&self, credentials: &Credentials) -> Result<AuthResult> {
        ...
        match self
            .client
            .authenticate_user(&auth_request.username, &auth_request.password)
        {
            Ok(ldap_user) => {
                info!(
                    "User {} authenticated successfully via LDAP",
    ...
    async fn validate_config(&self, config: &Value) -> Result<()> {
        let ldap_config: LdapConfig =
            serde_json::from_value(config.clone()).context("Invalid LDAP configuration format")?;
