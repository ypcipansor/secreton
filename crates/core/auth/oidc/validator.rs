...
impl OidcValidator {
    ...
    pub async fn validate_token(&self, token: &str) -> Result<ValidationResult> {
        ...
        // Get JWKS
        let jwks = self.get_jwks(&discovery.jwks_uri).await
            .context("Failed to get JWKS")?;
        
        // Find the right key
        let key = self.find_key(&jwks, &header)
        ...
        if is_valid {
            info!("JWT token validation successful for subject: {}", token_data.claims.sub);
        } else {
    ...
    async fn get_discovery(&self) -> Result<OidcDiscovery> {
        ...
        debug!("Fetching OIDC discovery document from: {}", self.config.discovery_url);
        ...
        info!("Successfully fetched OIDC discovery document for issuer: {}", discovery.issuer);
        Ok(discovery)
    }
    ...
    fn find_key(&self, jwks: &Jwks, header: &Header) -> Result<DecodingKey> {
        ...
        Err(anyhow!("No suitable key found in JWKS for algorithm: {}", alg_str))
    }
    ...
    fn jwk_to_decoding_key(&self, jwk: &Jwk) -> Result<DecodingKey> {
        match jwk.kty.as_str() {
            "RSA" => {
                let n = jwk.n.as_ref().ok_or_else(|| anyhow!("Missing 'n' parameter for RSA key"))?;
                let e = jwk.e.as_ref().ok_or_else(|| anyhow!("Missing 'e' parameter for RSA key"))?;
                
                // Decode base64url
                let _n_bytes = general_purpose::URL_SAFE_NO_PAD.decode(n)
                    .context("Failed to decode RSA modulus")?;
                let _e_bytes = general_purpose::URL_SAFE_NO_PAD.decode(e)
            ...
            "EC" => {
                ...
                let _crv = jwk.crv.as_ref().ok_or_else(|| anyhow!("Missing 'crv' parameter for EC key"))?;
                let _x = jwk.x.as_ref().ok_or_else(|| anyhow!("Missing 'x' parameter for EC key"))?;
                let _y = jwk.y.as_ref().ok_or_else(|| anyhow!("Missing 'y' parameter for EC key"))?;
                
                // TODO: Implement EC key support
                Err(anyhow!("EC keys not yet supported"))
            },
            "oct" => {
                let k = jwk.k.as_ref().ok_or_else(|| anyhow!("Missing 'k' parameter for symmetric key"))?;
    ...
    fn create_validation_settings(&self) -> Result<Validation> {
        ...
        // Set algorithms
        validation.algorithms = jwt_config.algorithms.iter()
            ...
            if let Some(scope_str) = scope.as_str() {
                ...
                for required_scope in &self.config.scopes {
                    if !token_scopes.contains(&required_scope.as_str()) {
                        return Err(anyhow!("Required scope '{}' not present in token", required_scope));
                    }
                }
            }
        }
    ...
    pub fn extract_user_info(&self, claims: &JwtClaims) -> Result<UserInfo> {
        ...
        let user_id = self.extract_claim_value(claims, &mapping.user_id_claim)
            .unwrap_or_else(|| claims.sub.clone());
            
        let username = self.extract_claim_value(claims, &mapping.username_claim)
        ...
        let groups = if let Some(groups_claim) = &mapping.groups_claim {
            self.extract_array_claim(claims, groups_claim).unwrap_or_default()
        } else {
            Vec::new()
        };
        
        let roles = if let Some(role_claim) = &mapping.role_claim {
            self.extract_array_claim(claims, role_claim).unwrap_or_default()
        } else {
            Vec::new()
        };
    ...
    fn extract_claim_value(&self, claims: &JwtClaims, claim_name: &str) -> Option<String> {
        match claim_name {
            ...
            _ => {
                claims.custom_claims.get(claim_name)
...
mod tests {
    ...
    fn test_extract_claim_value() {
        ...
        claims.custom_claims.insert("custom_field".to_string(), serde_json::Value::String("custom_value".to_string()));
        
        assert_eq!(validator.extract_claim_value(&claims, "sub"), Some("user123".to_string()));
        assert_eq!(validator.extract_claim_value(&claims, "email"), Some("user@example.com".to_string()));
        assert_eq!(validator.extract_claim_value(&claims, "custom_field"), Some("custom_value".to_string()));
