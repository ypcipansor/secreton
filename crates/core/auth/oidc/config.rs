...
impl OidcConfig {
    ...
    pub fn auth0(domain: &str, client_id: &str, client_secret: Option<&str>) -> Result<Self> {
        let discovery_url = Url::parse(&format!("https://{}/.well-known/openid_configuration", domain))?;
        ...
        config.scopes = vec!["openid".to_string(), "profile".to_string(), "email".to_string()];
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.jwt_validation.validate_aud && self.jwt_validation.audiences.is_empty() {
            return Err(anyhow!("Audiences must be specified when audience validation is enabled"));
        }
