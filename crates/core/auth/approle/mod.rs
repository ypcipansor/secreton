...
use super::traits::{AuthMethod, AuthResult as CoreAuthResult, Credentials, TokenInfo};
...
impl AppRoleAuth {
    ...
    pub fn new(storage: Arc<dyn StorageEngine>, audit_logger: AuditLogger) -> Self {
        Self {
            storage,
            audit_logger,
        }
    ...
    pub async fn create_role(&self, role_name: &str, role: AppRole) -> Result<()> {
    ...
    pub async fn generate_secret_id(
        ...
        let secret_hash = argon2
            .hash_password(secret_id.as_bytes(), &salt)
    ...
    pub async fn login(&self, role_id: &str, secret_id: &str) -> Result<LoginResponse> {
        // Get role name from role ID
        let role_id_key = format!("auth/approle/role-id/{}", role_id);
        let role_name_entry = self
            .storage
            .get(&role_id_key)
            .await?
        ...
        for full_accessor_key in self
            .storage
            .list("auth/approle/secret-id-accessor/")
            .await?
        {
            // Extract just the accessor UUID from the full key
            let accessor_result = full_accessor_key
                .strip_prefix("auth/approle/secret-id-accessor/")
            ...
            if let Some(_accessor_entry) = self.storage.get(&full_accessor_key).await? {
                ...
                if let Some(secret_entry) = self.storage.get(&secret_key).await? {
                    if let Ok(secret_id_data) =
                        serde_json::from_slice::<SecretId>(&secret_entry.value)
                    {
                        // Verify the secret ID hash
                        if let Ok(parsed_hash) = PasswordHash::new(&secret_id_data.secret_id_hash) {
                            let argon2 = Argon2::default();
                            if argon2
                                .verify_password(secret_id.as_bytes(), &parsed_hash)
                                .is_ok()
                            {
    ...
    pub async fn delete_role(&self, role_name: &str) -> Result<()> {
        ...
        let audit_log = crate::audit::AuditLog {
            ...
            ip: None,
            user_agent: None,
            metadata: [("role_name".to_string(), role_name.to_string())]
                .into_iter()
                .collect(),
        };
        self.audit_logger.log(audit_log).await?;
    ...
    pub async fn list_secret_id_accessors(&self, role_name: &str) -> Result<Vec<String>> {
    ...
    pub async fn destroy_secret_id(&self, accessor: &str) -> Result<()> {
        let secret_key = format!("auth/approle/secret-id/{}", accessor);
        let accessor_key = format!("auth/approle/secret-id-accessor/{}", accessor);
...
impl AuthMethod for AppRoleAuth {
    async fn authenticate(&self, credentials: &Credentials) -> Result<CoreAuthResult> {
        ...
        let (role_id, secret_id) = match credentials {
            ...
            _ => {
                return Err(anyhow!(
                    "Invalid credential type for AppRole authentication"
                ))
            }
        };

        match self.login(role_id, secret_id).await {
            Ok(response) => Ok(CoreAuthResult {
                success: true,
...
mod tests {
    ...
    async fn test_approle_authentication() {
        ...
        approle_auth
            .create_role("test-app", role_config)
            .await
            .unwrap();
        ...
        let secret_id = approle_auth
            .generate_secret_id("test-app", Some(metadata))
            .await
            .unwrap();
        ...
        // Test authentication
        let login_response = approle_auth
            .login(&role.role_id, &secret_id.secret_id_hash)
            .await
            .unwrap();
        ...
        assert_eq!(
            login_response.metadata.get("role_name").unwrap(),
            "test-app"
        );
    }
    ...
    async fn test_secret_id_expiration() {
        ...
        approle_auth
            ...
            .unwrap();
        let secret_id = approle_auth
            .generate_secret_id("test-app", None)
            .await
            .unwrap();
        let role = approle_auth.get_role("test-app").await.unwrap().unwrap();
