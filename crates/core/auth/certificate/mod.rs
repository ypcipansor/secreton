...
pub use validator::{CertificateInfo, CertificateValidator, OcspStatus, ValidationResult};
...
impl CertificateAuth {
    ...
    async fn validate_certificate(&self, creds: &CertificateCredentials) -> Result<User> {
        ...
        for required_ext in &self.config.required_extensions {
            if !creds.extensions.contains_key(required_ext) {
                return Err(anyhow!(
                    "Required certificate extension missing: {}",
                    required_ext
                ));
            }
        }
    ...
    async fn check_certificate_cache(
        &self,
        fingerprint: &str,
    ...
    async fn cache_certificate(&self, creds: &CertificateCredentials, user: &User) -> Result<()> {
        let cached = CachedCertificate {
            ...
            valid_until: creds
                .not_after
                .min(Utc::now() + chrono::Duration::seconds(self.config.cert_cache_ttl as i64)),
            cached_at: Utc::now(),
        };
    ...
    async fn get_user_by_id(&self, user_id: &str) -> Result<User> {
        ...
        match self.storage.get(&key).await? {
            Some(data) => {
                let user: User =
                    serde_json::from_slice(&data.value).context("Failed to deserialize user")?;
                Ok(user)
            }
...
impl AuthMethod for CertificateAuth {
    async fn authenticate(&self, credentials: &Credentials) -> Result<AuthResult> {
        let cert_creds = match credentials {
            ...
            _ => {
                return Err(anyhow!(
                    "Invalid credential type for certificate authentication"
                ))
            }
        };
        ...
        let token_info = TokenInfo {
            ...
            metadata: {
                ...
                meta.insert(
                    ...
                    cert_creds.fingerprint.clone(),
                );
                meta.insert(
                    "certificate_subject".to_string(),
                    cert_creds.subject.clone(),
                );
                meta
            },
