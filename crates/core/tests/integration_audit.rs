...
fn test_secret_versioning_and_revocation_audit() {
    ...
    let _v1 = history.put_secret("secret1".to_string(), "alice".to_string(), None, "audit1".to_string());
    history.log_audit_event(&mut audit_log, "put_secret", "alice", "created v1", "hash1", None);

    // Secret Versioning: update
    let _v2 = history.put_secret("secret2".to_string(), "bob".to_string(), Some("changed value".to_string()), "audit2".to_string());
