...
impl SecureStorage {
    ...
    fn derive_key(master_key: &[u8], salt: &[u8]) -> Result<[u8; KEY_LENGTH]> {
        ...
        argon2::hash_raw(master_key, salt, &config)
            .map_err(|e| anyhow!("Failed to derive key: {}", e))?
            .copy_into_slice(&mut key);
        Ok(key)
    }
    ...
    fn generate_key_entry(master_key: &[u8], metadata: Option<HashMap<String, String>>) -> Result<KeyEntry> {
    ...
    pub async fn new_with_keystore<K: KeyStore + 'static>(
        ...
        let current_key_entry = keys.get(&current_key_id)
            .ok_or_else(|| anyhow!("Current key not found in key store"))?;
        
        // Decode the key material
        let key_bytes = BASE64.decode(&current_key_entry.key)
            .map_err(|e| anyhow!("Failed to decode key material: {}", e))?;
        
        // Decode the salt
        let salt_bytes = BASE64.decode(&current_key_entry.salt)
    ...
    pub async fn encrypt(&self, data: &[u8]) -> Result<String> {
        ...
        let key_id_short = &self.current_key_id[..KEY_VERSION_LENGTH.min(self.current_key_id.len())];
        ...
        let ciphertext = self.current_cipher.encrypt(nonce, data)
    ...
    async fn maybe_rotate_key(&self) -> Result<()> {
        ...
        let current_key = keys.get_mut(&self.current_key_id)
    ...
    pub async fn rotate_key(&mut self) -> Result<()> {
        let current_key = self.keys.read().await
        ...
        let current_key_entry = keys.get(&self.current_key_id)
...
mod tests {
    ...
    async fn test_encrypt_decrypt() {
        let master_key = b"test-master-key";
        let secure_storage = SecureStorage::new(master_key, None).unwrap();
