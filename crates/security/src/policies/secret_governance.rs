...
impl GovernanceEngine {
    ...
    pub async fn check_compliance(
        ...
        for policy in policies.values() {
            ...
            let has_exception = exceptions.values().any(|_e| {
                _e.resource_id == resource_id
                    && _e.policy_id == policy.policy_id
                    && !_e.revoked
                    && Utc::now() < _e.expires_at
            });
            ...
            for condition in &policy.conditions {
                if let Some(value) = _context.get(&condition.field) {
                ...
                match policy.enforcement_level {
                    EnforcementLevel::Blocking => {
                        self.record_violation(resource_id, &policy.policy_id)
                            .await?;
                        return Err(GovernanceError::PolicyViolation(policy._name.clone()));
                    }
    ...
    pub async fn submit_approval(
        ...
        if workflow._status != WorkflowStatus::Pending
        ...
        if rejections_count > 0 {
            workflow._status = WorkflowStatus::Rejected;
    ...
    pub async fn get_analytics(&self) -> GovernanceAnalytics {
        ...
        let pending_approvals = workflows
            .values()
            .filter(|w| {
                w._status == WorkflowStatus::Pending || w._status == WorkflowStatus::InProgress
            })
            .count();

        let approved = workflows
            .values()
            .filter(|w| w._status == WorkflowStatus::Approved)
            .count();
...
mod tests {
    ...
    async fn test_create_policy() {
        ...
        let policy = GovernancePolicy {
            ...
            _name: "Require Approval".to_string(),
    ...
    async fn test_check_compliance() {
        ...
        let policy = GovernancePolicy {
            policy_id: "pol1".to_string(),
            _name: "Environment Check".to_string(),
        ...
        _context.insert("environment".to_string(), "production".to_string());

        let result = engine.check_compliance("resource1", &_context).await;
    ...
    async fn test_approval_workflow() {
        ...
        let workflow = ApprovalWorkflow {
            ...
            _name: "Secret Approval".to_string(),
            stages: vec![ApprovalStage {
                stage_id: "stage1".to_string(),
                _name: "Security Review".to_string(),
        ...
        assert_eq!(workflows[0]._status, WorkflowStatus::Approved);
    }
    ...
    async fn test_resolve_violation() {
        ...
        let mut _context = HashMap::new();
        _context.insert("owner".to_string(), "none".to_string());

        let _ = engine.check_compliance("res1", &_context).await;
