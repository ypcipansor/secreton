...
impl SecretScanner {
    ...
    fn get_remediation(&self, secret_type: &SecretType) -> String {
        match secret_type {
            SecretType::APIKey => {
                "Revoke the exposed API _key and generate a new one. Store in Vault.".to_string()
            }
            SecretType::Password => {
                "Reset the _password immediately. Use environment variables or Vault.".to_string()
            }
            SecretType::Token => {
            ...
            SecretType::DatabaseURL => {
                "Rotate database credentials. Use _connection pooling with Vault.".to_string()
            }
        }
    }
