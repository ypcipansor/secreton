#[cfg(test)]
mod tests {
    use crate::policies::policy::{PolicyContext, evaluate_with_sentinel};
    use crate::policies::sentinel::{EnforcementLevel, SentinelPolicy};
    ...
    const WASM_ALLOW: &[u8] = &[
        ...
        0x0f, // Type Section (ID 1, Size 15)
        0x03, // Count 3
    ...
    const WASM_DENY: &[u8] = &[
        ...
        0x60, 0x02, 0x7f, 0x7f, 0x01, 0x7f, // (i32, i32)->i32
        0x60, 0x00, 0x00, // ()->()
        0x03, 0x03, 0x02, 0x00, 0x01, 0x05, 0x03, 0x01, 0x00, 0x01, 0x07, 0x1d, 0x03, 0x06, 0x6d,
        0x65, 0x6d, 0x6f, 0x72, 0x79, 0x02, 0x00, 0x05, 0x61, 0x6c, 0x6c, 0x6f, 0x63, 0x00, 0x00,
        0x08, 0x65, 0x76, 0x61, 0x6c, 0x75, 0x61, 0x74, 0x65, 0x00, 0x01, 0x0a, 0x0b, 0x02, 0x04,
        0x00, 0x41, 0x00, 0x0b, 0x04, 0x00, 0x41, 0x00, 0x0b, // Func 1 body: const 0
    ];
    ...
    async fn test_evaluate_wasm_policy_placeholder_allow() {
        ...
        let result =
            evaluate_with_sentinel(&[policy], "user1", "secret/test", "read", &PolicyContext {})
                .await;
