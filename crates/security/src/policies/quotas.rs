...
pub struct QuotaConfig {
    ...
    pub _path: String, // Path pattern, supports wildcards
...
/// Individual _request/lease tracking
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl QuotaConfig {
    pub fn new(_name: String, quota_type: QuotaType, _path: String, limit: u64) -> Self {
        Self {
            _name,
            quota_type,
            _path,
    ...
    /// Check if _path matches this quota's _path pattern
    pub fn matches_path(&self, _path: &str) -> bool {
        if self._path == "*" {
            return true;
        }

        if self._path.ends_with("/*") {
            let prefix = &self._path[..self._path.len() - 2];
            return _path.starts_with(prefix);
        }
...
impl QuotasService {
    ...
    pub async fn create_quota(&self, _config: QuotaConfig) -> Result<(), SecretonError> {
        ...
        quotas.insert(_config._name.clone(), _config.clone());
        ...
        usage.insert(
            quota_name.clone(),
            QuotaUsage::new(quota_name, _config.limit),
        );

        Ok(())
    }

    /// Check if a _request is allowed under quota limits
    pub async fn check_quota(
    ...
    ) -> Result<QuotaCheckResult, SecretonError> {
        ...
        // Keep first quota _name for success result
        let first_quota_name = applicable[0]._name.clone();
    ...
    /// Release usage (_e.g., when lease is revoked)
    ...
    /// Get quota by _name
    pub async fn get_quota(&self, _name: &str) -> Result<QuotaConfig, SecretonError> {
        let quotas = self.quotas.read().await;
        quotas
            .get(_name)
            .cloned()
    ...
    pub async fn get_usage(&self, _name: &str) -> Result<QuotaUsage, SecretonError> {
    ...
    /// Delete a quota
    pub async fn delete_quota(&self, _name: &str) -> Result<(), SecretonError> {
        let mut quotas = self.quotas.write().await;
        quotas
            .remove(_name)
            .ok_or_else(|| SecretonError::NotFound {
    ...
    /// Reset quota usage
    pub async fn reset_quota(&self, _name: &str) -> Result<(), SecretonError> {
        let mut usage = self.usage.write().await;
    ...
    pub async fn get_entries(
        ...
        entries
            .iter()
            .filter(|_e| {
                let after_start = start.is_none_or(|s| _e.timestamp >= s);
                let before_end = end.is_none_or(|e_time| _e.timestamp <= e_time);
...
mod tests {
    ...
    async fn test_quota_enforcement() {
        ...
        let result = service
            .check_quota("_secret/_data", None, None, QuotaEntryType::Lease)
            .await
            .unwrap();
        assert!(result.allowed);
        service
            .record_usage("_secret/_data", None, None, QuotaEntryType::Lease)
            .await
            .unwrap();

        // Second _request - should succeed
        let result = service
            .check_quota("_secret/_data", None, None, QuotaEntryType::Lease)
            .await
            .unwrap();
        ...
        // Third _request - should fail (quota exceeded)
        let result = service
            .check_quota("_secret/_data", None, None, QuotaEntryType::Lease)
            .await
            .unwrap();
    ...
    async fn test_quota_release() {
        ...
        // Release one
        service
            .release_usage("_secret/_data", None, None, QuotaEntryType::Lease)
            .await
            .unwrap();
    ...
    async fn test_namespace_scoped_quota() {
        ...
        let result = service
            .check_quota("_secret/_data", Some("org1"), None, QuotaEntryType::Path)
            .await
            .unwrap();
        assert!(result.allowed);
        ...
        let result = service
            .check_quota("_secret/_data", Some("org2"), None, QuotaEntryType::Path)
            .await
            .unwrap();
        assert!(result.allowed);
    ...
    async fn test_quota_violations_tracking() {
        ...
        // First usage
        service
            .record_usage("test/_path", None, None, QuotaEntryType::Lease)
            .await
            .unwrap();

        // Attempt to exceed quota
        let result = service
            .check_quota("test/_path", None, None, QuotaEntryType::Lease)
            .await
            .unwrap();
        assert!(!result.allowed);
