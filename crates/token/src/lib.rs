//! # Secreton Token Management
//!
//! Unified token handling, JWT operations, and token lifecycle management
//! for the Secreton authentication system.
//!
//! This crate consolidates all JWT and token-related functionality
//! that was previously duplicated across multiple crates.
...
impl TokenService {
    ...
    pub fn create_access_token(
        ...
        let encoding_key = jsonwebtoken::EncodingKey::from_secret(self.config.jwt_secret.as_bytes());
    ...
    pub fn create_refresh_token(&self, user_id: &str, username: &str) -> Result<String, TokenError> {
        let now = Utc::now();
        let iat = now.timestamp() as usize;
        ...
        let header = jsonwebtoken::Header::new(jsonwebtoken::Algorithm::HS256);
        let encoding_key = jsonwebtoken::EncodingKey::from_secret(self.config.jwt_refresh_secret.as_bytes());
    ...
    pub fn create_token_pair(
        ...
        let access_token = self.create_access_token(user_id, username, email, roles, policies, mfa_required)?;
    ...
    pub fn validate_access_token(&self, token: &str) -> Result<AccessTokenClaims, TokenError> {
        ...
        let decoding_key = jsonwebtoken::DecodingKey::from_secret(self.config.jwt_secret.as_bytes());

        let token_data = jsonwebtoken::decode::<AccessTokenClaims>(token, &decoding_key, &validation)
            .map_err(TokenError::JwtError)?;
    ...
    pub fn validate_refresh_token(&self, token: &str) -> Result<RefreshTokenClaims, TokenError> {
        let validation = jsonwebtoken::Validation::new(jsonwebtoken::Algorithm::HS256);
        let decoding_key = jsonwebtoken::DecodingKey::from_secret(self.config.jwt_refresh_secret.as_bytes());

        let token_data = jsonwebtoken::decode::<RefreshTokenClaims>(token, &decoding_key, &validation)
            .map_err(TokenError::JwtError)?;
