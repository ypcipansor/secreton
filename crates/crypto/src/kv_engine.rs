...
impl KVEngine {
    ...
    pub async fn get_secret(
        ...
        path: &str,
        version: Option<u32>,
    ) -> CryptoResult<SecretData> {
        let secrets = self.secrets.read().unwrap();
        let path_secrets = secrets.get(path)
        ...
        let secret_version = if let Some(v) = version {
            ...
            let latest_version = path_secrets.keys().max()
                .ok_or_else(|| CryptoError::KeyNotFound(path.to_string()))?;
    ...
    pub async fn delete_secret(&self, path: &str, version: Option<u32>) -> CryptoResult<()> {
        let mut secrets = self.secrets.write().unwrap();
        let path_secrets = secrets.get_mut(path)
            .ok_or_else(|| CryptoError::KeyNotFound(path.to_string()))?;
