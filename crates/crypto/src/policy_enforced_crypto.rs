...
use crate::crypto_policy_engine::{ComplianceStandard, CryptoPolicyEngine};
use thiserror::Error;
...
impl PolicyEnforcedCryptoOperations {
    ...
    pub async fn encrypt_with_policy(
        ...
        let (selected_algorithm, _compliance_standards) = {
            ...
            let compliance_standards = policy_engine
                .check_compliance(ComplianceStandard::Fips140_3)
                .await;
        ...
        let encrypted_secret = EncryptedSecret {
            ...
            compliance_standards: vec![ComplianceStandard::Fips140_3],
...
mod tests {
    use super::*;
    use crate::{AlgorithmMetadata, AlgorithmStatus, CryptoPolicy};
        ...
        // Register ChaCha20Poly1305 as approved algorithm
        {
            ...
            policy_engine
                .register_algorithm(AlgorithmMetadata {
                    ...
                    security_level: 256,
                    compliance: vec![ComplianceStandard::Fips140_3],
    ...
    async fn test_compliance_scan() {
        ...
        let policy = CryptoPolicy {
            ...
            name: "Test Policy".to_string(),
            allowed_algorithms: vec![CryptoAlgorithm::Aes256Gcm],
            min_key_sizes: HashMap::from([("Aes256Gcm".to_string(), 256)]),
            compliance_standards: vec![ComplianceStandard::Fips140_3],
    ...
    async fn test_migration_plan_creation() {
        ...
        let plan = ops
            .create_migration_plan(CryptoAlgorithm::Aes128Gcm, CryptoAlgorithm::Aes256Gcm)
            .await
            .unwrap();

        assert_eq!(plan.from_algorithm, CryptoAlgorithm::Aes128Gcm);
        assert_eq!(plan.to_algorithm, CryptoAlgorithm::Aes256Gcm);
    ...
    async fn test_violation_tracking() {
        ...
        let policy = CryptoPolicy {
            ...
            allowed_algorithms: vec![CryptoAlgorithm::Aes256Gcm, CryptoAlgorithm::ED25519],
