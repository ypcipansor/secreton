...
/// Type of key that can be rotated
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub enum KeyType {
    /// Transit encryption key
    Transit,
    /// PKI certificate authority key
    PKI,
    /// Token signing key
    TokenSigning,
    /// Seal/unseal key
...
impl KeyVersion {
    ...
    pub fn destroy(&mut self) {
        ...
        self.is_active = false;
        self.key_data.clear(); // Securely wipe key data
    }
...
pub struct KeyRotationService {
    ...
    status: Arc<RwLock<HashMap<String, RotationStatus>>>,
}
...
impl KeyRotationService {
    ...
    pub async fn register_key(&self, config: KeyRotationConfig) -> Result<ManagedKey> {
        ...
        let key_type = config.key_type.clone();
        let managed_key = ManagedKey::new(key_name.clone(), key_type, config);
        ...
        status.insert(key_name, RotationStatus::Idle);
    ...
    pub async fn rotate_key(&self, key_name: &str, trigger: RotationTrigger) -> Result<u64> {
        // Check if rotation is already in progress
        {
            ...
            if let Some(s) = status.get(key_name) {
        ...
        {
            let mut status = self.status.write().await;
            status.insert(key_name.to_string(), RotationStatus::InProgress);
        }
        ...
        if managed_key.config.min_encryption_version > old_version {
        ...
        completed_entry.status = RotationStatus::Completed;
        history.push(completed_entry);

        // Update status
        let mut status = self.status.write().await;
        status.insert(key_name.to_string(), RotationStatus::Completed);

        Ok(new_version)
    }

    /// Record key operation (for usage-based rotation)
    ...
    /// Check if key should be rotated and trigger if auto-rotate enabled
    ...
    /// Deprecate a specific key version
    ...
    /// Destroy a specific key version (irreversible)
    pub async fn destroy_version(&self, key_name: &str, version: u64) -> Result<()> {
        ...
        if !managed_key.config.deletion_allowed {
            return Err(KeyRotationError::InvalidConfig(
                "Deletion not allowed for this key".to_string(),
            ));
        }
    ...
    pub async fn update_config(&self, key_name: &str, config: KeyRotationConfig) -> Result<()> {
        let mut keys = self.keys.write().await;
        let managed_key = keys
            .get_mut(key_name)
            .ok_or_else(|| KeyRotationError::NotFound(key_name.to_string()))?;

        managed_key.config = config;
        managed_key.config.updated_at = Utc::now();
...
mod tests {
    ...
    async fn test_register_key() {
        ...
        let key = service.register_key(config).await.unwrap();
        assert_eq!(key.name, "transit-key-1");
    ...
    async fn test_manual_rotation() {
        ...
        let key = service.get_key("test-key").await.unwrap();
        assert_eq!(key.current_version, 2);
        assert_eq!(key.versions.len(), 2);
        assert!(key.latest_rotation.is_some());
    }
    ...
    async fn test_usage_based_rotation() {
        ...
        // Record operations
        for _ in 0..3 {
            service.record_operation("usage-key", 1).await.unwrap();
        }

        // Should trigger rotation
        let result = service.check_and_rotate("usage-key").await.unwrap();
    ...
    async fn test_deprecate_version() {
        ...
        let key = service.get_key("test-key").await.unwrap();
        let version_1 = key.get_version(1).unwrap();
