...
pub fn compute_hash_multiple(algorithm: AlgorithmId, data_chunks: &[&[u8]]) -> CryptoResult<HashResult> {
    let hash = match algorithm {
        AlgorithmId::Sha256 => {
            let hasher = Sha256Hash;
...
pub mod password {
    ...
    pub fn hash_password_pbkdf2(password: &str, salt: &[u8], iterations: u32) -> CryptoResult<Vec<u8>> {
