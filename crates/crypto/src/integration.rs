...
//! and the existing Secreton core system, including API endpoints,
//! storage backend, and service management.

use crate::error::{CryptoError, CryptoResult};
use crate::transit::{BatchOperation, KeyOptions, KeyType, TransitEngine, TransitPolicies};
use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};
use std::sync::Arc;
use tokio::sync::RwLock;

/// Integration wrapper for the transit engine with Secreton core services
...
pub enum StorageBackend {
    ...
    File { path: String },
    Database { url: String },
    Secret { address: String, token: String },
...
impl TransitIntegration {
    ...
    fn create_policies_from_config(config: &IntegrationConfig) -> TransitPolicies {
        ...
        policies.performance_limits.max_batch_size = config.api.max_batch_size;
        ...
        match &self.config.storage.backend {
            StorageBackend::File { path } => {
                tokio::fs::create_dir_all(path).await.map_err(|e| {
                    CryptoError::StorageError(format!("Failed to create storage directory: {}", e))
                })?;
            }
            ...
            StorageBackend::Secret {
                ...
                // This would integrate with HashiCorp Secret
            }
    ...
    pub async fn create_key(
        ...
        self.validate_key_creation(&name, &key_type, &options)
            .await?;

        // Create key through transit engine
        let result = self
            .engine
            .create_key(name.clone(), key_type, options)
            .await;
    ...
    pub async fn encrypt(
        ...
        // Validate request
        self.validate_encryption_request(key_name, plaintext)
            .await?;

        // Perform encryption
        let result = self
            .engine
            .encrypt(key_name, plaintext, context, key_version)
            .await;
    ...
    pub async fn decrypt(
        ...
        // Validate request
        self.validate_decryption_request(key_name, ciphertext)
            .await?;
    ...
    pub async fn batch_operation(
        ...
        if operations.len() > self.config.api.max_batch_size {
            return Err(CryptoError::ValidationError(format!(
                ...
                operations.len(),
                self.config.api.max_batch_size
            )));
        }
    ...
    async fn validate_key_creation(
        ...
        match key_type {
            ...
            KeyType::EcdsaP256 | KeyType::EcdsaSecp256k1 | KeyType::Ed25519 | KeyType::X25519 => {
                Ok(())
            }
        }
    }
    ...
    async fn validate_encryption_request(
        ...
        if plaintext.len() > self.config.api.max_request_size {
            return Err(CryptoError::ValidationError(format!(
                ...
                plaintext.len(),
                self.config.api.max_request_size
            )));
        }
...
/// Extension trait for Secreton core integration
pub trait SecretonTransitIntegration {
    /// Add transit engine to Secreton core services
    fn add_transit_engine(&mut self, integration: TransitIntegration) -> CryptoResult<()>;

    /// Get transit engine from Secreton core services
...
mod tests {
    ...
    async fn test_transit_integration() {
        ...
        let encrypt_response = integration
            .encrypt("test-key", plaintext, None, None)
            .await
            .unwrap();

        // Test decryption
        let decrypt_response = integration
            .decrypt("test-key", &encrypt_response.ciphertext, None)
            .await
            .unwrap();
    ...
    async fn test_validation() {
        ...
        let result = integration
            .validate_key_creation("", &KeyType::Aes256Gcm, &None)
            .await;
        ...
        let result = integration
            .validate_encryption_request("test-key", &large_data)
            .await;
        assert!(result.is_err());
    }
}
