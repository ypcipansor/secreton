...
impl KdfParams {
    ...
    pub fn argon2id(memory_cost: u32, iterations: u32, parallelism: u32, key_length: usize) -> CryptoResult<Self> {
        let salt = generate_random_bytes(16)?;
        Ok(Self {
            ...
            AlgorithmId::Pbkdf2 => {
                if self.iterations < 100_000 {
                    return Err(CryptoError::KeyGenerationFailed {
                        reason: format!("PBKDF2 iterations too low: {} (minimum 100,000)", self.iterations),
                    });
                }
            }
...
pub fn derive_key_pbkdf2(password: &[u8], salt: &[u8], iterations: u32, key_length: usize) -> CryptoResult<Vec<u8>> {
...
pub fn derive_key_argon2id(
    ...
    let params = Params::new(memory_cost, iterations, parallelism, Some(key_length))
...
pub mod stretch {
    ...
    pub fn derive_multiple_keys(master_key: &[u8], info_list: &[&str], key_length: usize) -> CryptoResult<Vec<Vec<u8>>> {
