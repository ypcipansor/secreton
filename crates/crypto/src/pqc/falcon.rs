        ...
        match self.variant {
            ...
            FalconVariant::Falcon1024 => {
                let sk = falcon1024::SecretKey::from_bytes(&self.private_key)
                    .map_err(|_| CryptoError::InvalidKey("Invalid Falcon-1024 private key".to_string()))?;
        ...
        match self.variant {
            FalconVariant::Falcon512 => {
                let pk = falcon512::PublicKey::from_bytes(&self.public_key)
                    .map_err(|_| CryptoError::InvalidKey("Invalid Falcon-512 public key".to_string()))?;
                let detached_sig = falcon512::DetachedSignature::from_bytes(signature)
                    .map_err(|_| CryptoError::InvalidSignature("Invalid Falcon-512 signature".to_string()))?;
            ...
            FalconVariant::Falcon1024 => {
                let pk = falcon1024::PublicKey::from_bytes(&self.public_key)
                    .map_err(|_| CryptoError::InvalidKey("Invalid Falcon-1024 public key".to_string()))?;
                let detached_sig = falcon1024::DetachedSignature::from_bytes(signature)
                    .map_err(|_| CryptoError::InvalidSignature("Invalid Falcon-1024 signature".to_string()))?;
...
impl FalconBatchSigner {
    ...
    pub fn new(private_key: Vec<u8>, variant: FalconVariant) -> Self {
        Self {
            private_key,
            variant,
        }
...
mod tests {
    ...
    fn test_batch_operations() {
        ...
        let batch_signer = FalconBatchSigner::new(keypair.private_key.clone(), FalconVariant::Falcon512);
        let signatures = batch_signer.batch_sign(&messages).unwrap();

        assert_eq!(signatures.len(), 3);
        assert!(signatures.iter().all(|sig| sig.len() == 690));

        let batch_verifier = FalconBatchVerifier::new(keypair.public_key, FalconVariant::Falcon512);
        let results = batch_verifier.batch_verify(&messages, &signatures.iter().map(|s| s.as_slice()).collect::<Vec<_>>()).unwrap();
    ...
    fn test_signature_size_comparison() {
        ...
        let falcon_128 = sizes.iter().find(|(name, _, _)| name == "Falcon-512").unwrap();
        let falcon_256 = sizes.iter().find(|(name, _, _)| name == "Falcon-1024").unwrap();
        let mldsa_128 = sizes.iter().find(|(name, _, _)| name == "ML-DSA-44").unwrap();
        let mldsa_256 = sizes.iter().find(|(name, _, _)| name == "ML-DSA-87").unwrap();
