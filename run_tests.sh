...
# Brankas Development and Testing Script
# This script sets up, builds, and tests the complete Brankas transit engine system
...
echo -e "${CYAN}🔐 Brankas Transit Engine - Development and Testing Suite${NC}"
echo "======================================================================"

# Check if we're in the right directory
if [ ! -f "Cargo.toml" ] || [ ! -d "crates" ]; then
    echo -e "${RED}❌ Error: Must be run from the brankas root directory${NC}"
    exit 1
fi
...
# Build core crate
run_command "Building core crate" "cargo build -p brankas-core"

# Build simple crate
run_command "Building simple crate" "cargo build -p brankas-simple"

# Build agent crate  
run_command "Building agent crate" "cargo build -p brankas-agent"

# Build API crate
run_command "Building API crate" "cargo build -p brankas-api"

# Build API server binary
run_command "Building API server binary" "cargo build -p brankas-api --bin api_server"

# Build UI crate
run_command "Building UI crate" "cargo build -p brankas-ui"
...
# Run core tests
run_command "Running core tests" "cargo test -p brankas-core"

# Run simple tests
run_command "Running simple tests" "cargo test -p brankas-simple"

# Run agent tests
run_command "Running agent tests" "cargo test -p brankas-agent"

# Run API tests
run_command "Running API tests" "cargo test -p brankas-api"

# Run UI tests
run_command "Running UI tests" "cargo test -p brankas-ui"
...
echo
echo -e "${CYAN}🎉 Brankas Transit Engine is ready for production use!${NC}"
echo
echo "Next steps:"
echo "1. Deploy API server: cargo run -p brankas-api --bin api_server"
...
echo
echo "Documentation available at: target/doc/brankas_*/index.html"
echo "======================================================================"
