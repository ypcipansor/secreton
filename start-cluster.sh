...
# Brankas Vault - Multi-Node Raft Cluster Setup
# This script sets up a 3-node Raft cluster for high availability

set -e

echo "🏛️  Setting up Brankas Vault 3-Node Raft Cluster"
echo "=============================================="

# Configuration
CLUSTER_NAME="${CLUSTER_NAME:-brankas-cluster}"
...
for i in "${!NODES[@]}"; do
    ...
    export BRANKAS_NODE_ID="$node_id"
    export BRANKAS_RAFT_DATA_DIR="$BASE_DATA_DIR/$node_id"
    export BRANKAS_RAFT_BIND_ADDR="$raft_host:$raft_port"
    export BRANKAS_RAFT_ADVERTISE_ADDR="$raft_host:$raft_port"
    export BRANKAS_RAFT_PEERS="$peers"
    export BRANKAS_HOST="$api_host"
    export BRANKAS_PORT="$api_port"
    export BRANKAS_LOG_LEVEL="info"
    
    # Start node in background
    cargo run -p brankas-api --bin api_server > "./logs/$node_id/server.log" 2>&1 &
