...
use secreton_crypto::key_derivation;
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(password) = std::str::from_utf8(data) {
        let _pbkdf2_key = key_derivation::presets::derive_aes256_pbkdf2(password);
    ...
    let _argon2_fast = key_derivation::presets::derive_aes256_argon2_fast("test_password");
    let _argon2_secure = key_derivation::presets::derive_aes256_argon2_secure("test_password");
    let _chacha_argon2 = key_derivation::presets::derive_chacha20_argon2("test_password");

    // Test key stretching
    let _stretched = key_derivation::stretch::stretch_key_sha256(data, 1000);

    // Test multiple key derivation
    let info_list = &["encryption", "authentication", "signing"];
    let _multiple_keys = key_derivation::stretch::derive_multiple_keys(data, info_list, 32);
