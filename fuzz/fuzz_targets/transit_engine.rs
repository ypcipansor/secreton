...
fuzz_target!(|data: &[u8]| {
    ...
    for key_type in &key_types {
        ...
        let _create_result = tokio::runtime::Runtime::new()
            .unwrap()
            .block_on(async {
                engine.create_key(key_name.clone(), key_type.clone(), None).await
            });
    }

    // Test encryption/decryption with different keys
    let test_key = "test-encryption-key";
    let _create_encrypt_key = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async {
            engine.create_key(test_key.to_string(), KeyType::Aes256Gcm, None).await
        });
    ...
    if !plaintext.is_empty() {
        let _encrypt_result = tokio::runtime::Runtime::new()
            .unwrap()
            .block_on(async {
                engine.encrypt(test_key, plaintext, None, None).await
            });
        ...
        if let Ok(ciphertext) = tokio::runtime::Runtime::new()
            ...
            let _decrypt_result = tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(async {
                    engine.decrypt(test_key, &ciphertext, None, None).await
                });
        }
    }
    ...
    let _create_sign_key = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async {
            engine.create_key(sign_key.to_string(), KeyType::Ed25519, None).await
        });

    let _sign_result = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async {
            engine.sign(sign_key, data, None, None).await
        });
    ...
    if let Ok(signature) = tokio::runtime::Runtime::new()
        ...
        let _verify_result = tokio::runtime::Runtime::new()
            .unwrap()
            .block_on(async {
                engine.verify(sign_key, data, &signature, None, None).await
            });
    }
    ...
    let _rotate_result = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async {
            engine.rotate_key(test_key).await
        });
    ...
    let _random_bytes = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async {
            engine.random(data.len().min(1024)).await
        });
    ...
    let _derived_key = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async {
            engine.derive_key("derived-key", data, Some(&data[0..16])).await
        });
});