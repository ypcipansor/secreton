...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(key_name) = std::str::from_utf8(data) {
        if !key_name.is_empty() && key_name.len() < 64 {
            ...
            let _create_result = tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(async {
                    engine.create_key(key_name.to_string(), KeyType::X25519, None).await
                });
            ...
            let _key_info = tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(async {
                    engine.get_key_info(key_name).await
                });
            ...
            let _rotate_result = tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(async {
                    engine.rotate_key(key_name).await
                });
    ...
    let _derive_result = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async {
            engine.derive_key("test-x25519-key", data, 32).await
        });
});