...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(encrypted) = CryptoEngine::new().encrypt(AlgorithmId::Aes256Gcm, plaintext, key) {
        if let Ok(decrypted) = CryptoEngine::new().decrypt(&encrypted, key) {
    ...
    if let Ok(encrypted) =
        CryptoEngine::new().encrypt(AlgorithmId::ChaCha20Poly1305, plaintext, key)
    {
        if let Ok(decrypted) = CryptoEngine::new().decrypt(&encrypted, key) {
            // Verify roundtrip correctness
            assert_eq!(
